(function (_aoWin) {
    ...
    _oQMAddress._initQuickAddr = function (_aoOption) {
        ...
        var _nDISPLAYHOT = 9, _oAllAddressList = _oQMAddress._moAddress._oAllAddressList,
            _oAddrs = _aoOption.sortbyupdatetime, _oId2EmailCache = _oQMAddress._moAddress.id2email,
            _oEmail2IdCache = _oQMAddress._moAddress.email2id, _oItem;
        ...
        for (var i = 0, _nLen = _oAddrs.length; i < _nLen; i++) {
            if (_oItem = _oAddrs[i]) {
                _oQMAddress.addAddress({
                    id: _oItem[0],
                    pinyin: _oItem[1],
                    email: _oItem[2],
                    name: _oItem[8] ? _oItem[8] : _oItem[3],
                    qq: _oItem[4],
                    acspec: _oItem[3].indexOf('&') >= 0,
                    u: _oItem[7],
                    memo: _oItem[8] ? _oItem[3] : _oItem[8]
                });
            }
        }
        var _oMailMainGroup = {
        ...
        for (var i = 0, _nLen = _aoOption.groups.length; i < _nLen; i++) {
            ...
            if (_oIds.length > 0 && _oIds.length < 21) {
                ...
                _oQMAddress.addAddress({
                    nShortcutGroupId: _oGroup[0],
                    id: _nShortcutGroupId++,
                    pinyin: _oGroup[3],
                    email: _oGroupName.join("; "),
                    name: _oGroup[2],
                    qq: -1,
                    acspec: _oGroup[2].indexOf('&') >= 0
                });
            }
            var _oGroupObj = {
                id: _oGroup[0],
        ...
        for (var i = 0, _nLen = _aoOption.domaingroups.length; i < _nLen; i++) {
            ...
            var _oGroupObj = {
                id: _aoOption.domaingroups[i][0],
                name: _aoOption.domaingroups[i][2],
                addressesId: _oIds,
                groupsId: [],
                parentId: _oDomainGroup.id,
                pinyin: _aoOption.domaingroups[i][3]
            };
        ...
        if (_aoOption.ggroup) {
            for (var i = 0, _nLen = _aoOption.ggroup.length; i < _nLen; i++) {
                ...
                if (!_oAddr) {
                    ...
                    _oQMAddress.addAddress(_oAddr = {
                        id: _nShortcutGroupId,
                        pinyin: "",
                        email: _aoOption.ggroup[i][2],
                        name: _aoOption.ggroup[i][2].replace(/@.*/, ""),
                        qq: g_admuin,
                        acspec: false
                    });
                }
        ...
        _oQMAddress.addGroup({
            id: "myself",
            name: "\u6211\u81EA\u5DF1\u7684\u90AE\u7BB1",
            groupType: "myself",
            addressesId: [_sMyUniKey],
            groupsId: [],
            parentId: 0,
            pinyin: "WOZIJIDEYOUXIANG"
        });
        _oQMAddress.addAddress({
            id: _sMyUniKey,
            email: getTop().getUserInfoText("addr"),
            name: "\u6211\u81EA\u5DF1\u7684\u90AE\u7BB1",
            isMyself: true,
            acspec: false,
            istool: true
    ...
    _oQMAddress._loadRecentGroupData = function () {
        ...
        QMAjax.send(_sUrl, {
            onload: function (_abIsOk, _asParam, _aoXmlObj) {
                if (_abIsOk) {
                    var _oData;
                    try {
    ...
    _oQMAddress._loadAddressDataCallback = function (_asType, _asMsg) {
        if (_oQMAddress.isInit()) {
            _oQMAddress._loadAddressData._fire({
                sType: 'succeed',
                sMsg: '\u540C\u6B65QQ\u597D\u53CB\u5230\u8054\u7CFB\u4EBA\u4E2D'
            });
        }
    ...
    _oQMAddress.getRecentGroup = function (_aoConfig) {
        var _sId = _aoConfig.searchId, _sSearchStr = getTop().htmlEncode(_aoConfig.searchStr),
            _oAddrList = _aoConfig.addrList, _oRecentGroup = _oQMAddress._moRecentGroup,
            _oMatchReg = new RegExp(["<(\\d+)>[^>]*", regFilter(_sSearchStr), "[^>]*>;"].join("")), _oFilterGroups = [],
            _oResultGroups = [];
        E(_oRecentGroup, function (_aoGroupItem, _anGroupIdx) {
            var _oTmpGroup = {
                index: _aoGroupItem.index,
                total: _aoGroupItem.total,
                matchReg: _aoGroupItem.matchReg,
                nicksStr: _aoGroupItem.nicksStr[_sId],
                matchNicks: {},
                matchAddrs: {},
                hasMatched: {}
            }, _bMatch = true;
            E(_oRecentGroupConf, function (_asId) {
                ...
                for (var _nAddrIdx = 0; _nAddrIdx < _oAddrList[_asId].length; ++_nAddrIdx) {
                    var _sAddrReg = new RegExp(["<(\\d+)>\"([^\"]*)\"<", regFilter(_oAddrList[_asId][_nAddrIdx]), ">;"].join("")),
                        _oTestRes = _sAddrReg.exec(_aoGroupItem.matchReg[_asId]);
                    if (_oTestRes) {
        ...
        E(_oFilterGroups, function (_oItem, _nIdx) {
            ...
            if (_oTestRes) {
                var _bHasMatched = false, _nIdx = _oTestRes[1], _sTmpStr,
                    _sName = getTop().htmlEncode(_oRecentGroup[_oItem.index].data[_sId][_nIdx].name), _nShowCnt = 0,
                    _oShowNicks = [];
                ...
                var _oBoldReg = new RegExp("(" + regFilter(_sSearchStr) + ")"), _oReplace = "<b>$1</b>",
                    _nNickCnt = _nLen;
    ...
    _aoWin.getAddrACData = function (_aoInputObj, _asDefaultValue, _afFilterFunc) {
        ...
        var _asFilter = trim(_sAddrValue).toLowerCase(),
            _sGroupImgage = '<img unselectable="on" class="lm_groupAutoIcon" src="' + getPath('image') + 'spacer.gif" title="\u90AE\u4EF6\u7EC4\u5E10\u53F7"/>',
            _oSelf = arguments.callee, _oAddrList, _oRegExp = new RegExp("(" + regFilter(_asFilter) + ")", "i"),
            _sReplace = '<b unselectable="on">$1</b>', _oCacheAddress = [], _oOutput = [], _oOutputMap = {},
            _oExistMap = {}, _nOutputIndex, _sFilterLen = _asFilter.length;
        ...
        while (_nOutputIndex >= 0) {
            var _oItem = _oOutput[_nOutputIndex], _oAddrData = _oItem.oAddress,
                _sItemValueTemp = ['"', _oAddrData.name, '" &lt;', _oAddrData.email, '&gt;'].join("");
