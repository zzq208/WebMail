(function (_aoLoadWin) {
    var _ID = {
            ...
            _sQSource: "source",
            _sQSendBtn: "sendbtn",
            _sQReplyTxtContainer: "qmQuickReplyTextContainer",
            _sQReplyBtnContainer: "qmQuickReplyButtonContainer",
            _sAfterSendDiv: "afterSendDiv",
            _sAfterSendingDiv: "afterSendingDiv",
            _sQuickReplyPart: "QuickReplyPart",
            _sQMEditorArea: "QMEditorArea",
            _sQMEditorToolArea: "QMEditorToolBarPlusArea",
            _sJumpToNewWin: "jumpToNewWin",
            _sReplyFrm: "replyfrm",
            _sSubMailStartDiv: "submail_start_div",
            _sContentDiv: "contentDiv",
            _sContentClassName: "qm_converstaion_body",
            _sImgStar: "img_star",
            _sTopBtn: "img_mailtop",
            _sStarStatus: "starStatus",
            _sMailtopStatus: "mailtopStatus",
            _sStarTopStatus: "starTopStatus",
            _sTagContainer: 'tagContainer',
            _sMailContainer: "mailContentContainer",
            _sSendTimeMsg: "send_time_msg",
            _sAttachment: "attachment",
            _sPageEnd: "pageEnd",
            _sNextMailTop: "nextmail_top",
            _sNextMailBt: "nextmail_bt",
            _sNextNewMail: "nextnewmail",
            _sNextNewDiv: "nextnewDiv",
            _sQQMailBgMusicInfo: "QQMailBgMusicInfo",
            _sBgMusic: "bgmusic",
            _sMp3PlayerContainer: "mp3player_container",
            _sMp3PlayerInfo: "mp3player_info",
            _sSubMail: "submail",
            _sFold: "fold",
            _sDetail: "detail",
            _sSum: "sum",
            _sExpand: "expand",
            _sDetailBtn: "detailBtn",
            _sReferInfo: "referinfo",
            _sSettingGroup: "setting_group",
            _sRmd: "rmd",
            _sRemarkContent: "remarkContent",
            _sRemarkContainer: "remarkcontainer",
            _sRemarkText: "remarktext",
            _sRemarkWrite: "remarkwrite",
            _sRemarkRead: "remarkread",
            _sRemarkSave: "remarksave",
            _sSenderInfo: "senderInfo",
            _sSenderInfo2: "senderInfo2",
            _sSenderInfo3: "senderInfo3",
            _sMainMail: "mainmail",
            _sRejectGroupY: "rejectgroupy",
            _sRejectGroupN: "rejectgroupn",
            _sCheatCodeBar: "spam",
            _sGreenBar: "cheatcode_greenbar",
            _sGreenBarText: "greenbar_text"
        }, _NAME = {
            _sConvReAndFw: "convreandfw",
            _sSpam: "spam",
            _sAdMail: "adMail",
            _sMoreOprContainer: "moreoprContainer"
        }, _sQMTRANSLATE = "qmTranslate", _DEFAULT_TEXT_VALUE = "\u8BF7\u5728\u6B64\u8F93\u5165\u5907\u6CE8...",
        _MUSICINFO = T(['<img src="$images_path$spacer1e9c5d.gif" class="icon_addMusic_d"/>', '&nbsp;<a href="http://cgi.music.soso.com/fcgi-bin/m.q?w=$title$%20$author$&t=0" target="_blank">$title$ - $author$</a>']),
        _MAIL_LIST1 = TE(["/cgi-bin/mail_list?sid=$sid$&folderid=$folderid$&folderkey=$folderkey$", '$@$if($s$=="from_unread_list")$@$', '&flag=new&s=unread', '$@$else if($s$=="from_star_list")$@$', '&flag=star&s=star', '$@$endif$@$', '$@$if($more$)$@$', '$more$', '$@$endif$@$']),
        _READMAIL_ = "/cgi-bin/readmail?sid=$sid$&mailid=$mailid$",
        _READMAIL_CPS_URL = TE([_READMAIL_, "&t=", "$@$if($t$)$@$", "$t$", "$@$else$@$", "compose", "$@$endif$@$", "&s=$s$&disptype=$disptype$"]),
        _ATT_FW_SEND = TE(['/cgi-bin/readmail?sid=$sid$&t=compose&s=forward&disptype=html&from=attachfolder&getSubjectFromAttachName=1', '&mailattach=$mailid$|$attachid$|$attachname$|$fileextenal$|$filebyte$']),
        _BIG_ATT_FW_SEND = TE(["/cgi-bin/readtemplate?sid=$sid$&t=compose&s=cnew&bigatt=1&attid=$attachid$&filesize=$filesize$&downpage=$downpage$&exptime=$exptime$&nocheckframe=true&urifilename=$urifilename$&getSubjectFromAttachName=1&k=$k$&code=$code$&fid=$fid$"]),
        _SETTING_GROUP = TE("/cgi-bin/grouplist?oper=$oper$&sid=$sid$&gid=$gid$@groupmail.qq.com&t=mail_mgr2&mailaction=$action$"),
        _REJECT_GROUP = TE("/cgi-bin/grouplist?oper=reject&sid=$sid$&reject=$yn$&gid=$gid$@groupmail.qq.com&t=mail_mgr2&mailaction=reject_group"),
        _S_RD_LIST = TE("/cgi-bin/reader_list?type=home&classtype=allfriend&uin=$mail.from.qq$&t=reader_personal&sid=$sid$&s=sidebar"),
        _S_ML_LIST = TE("/cgi-bin/mail_list?sid=$sid$&t=mail_list_preview&sender=$mail.from.addr$&receiver=$mail.from.addr$&matchtype=include&folderid=all&page=0&name=$mail.from.name$&pagesize=5&mailidx=$mailid$&listmode=0&from=sidebar&s=$s$&record=n"),
        _FORM_TMP = TE(['<form method="$sMethod$" id="$sFormId$" name="$sFormId$" target="$sTarget$" action="$sAction$" enctype="multipart/form-data">', '$@$for($oInputs$)$@$', '<input name="$name$" type="hidden" value="$value$"/>', '$@$endfor$@$', '</form>']),
        _ADDR_ = '"$@$eval getTop().htmlEncode($name$)$@$"&lt;$addr$&gt;; ',
        _ADDRS = TE(['$@$for($addrs$)$@$', '"$@$eval getTop().encodeNick($name$)$@$"<$addr$>; ', '$@$endfor$@$']),
        _SETTING_GROUP_TMP = TE(['<div id="container" style="line-height:20px">', '<div>', '<div style="margin:8px 4px 0 0;">', '<a class="right" id="other" href="/cgi-bin/grouplist?sid=$sid$&t=setting_group&oper=list">\u7BA1\u7406\u5176\u4ED6\u7FA4</a>', '<p style="margin:0;">\u5BF9\u6B64\u7FA4:</p>', '</div>', '<div style="margin-bottom:5px;margin-left:8px;">', '<div style="padding:5px 0 3px">', '<input type="radio" name="qqnotify" class="cb" value="notifyyes" $@$if(!$bReject$&&$bNotify$)$@$checked="true"$@$endif$@$ id="qqnotify"/>', '<label for="qqnotify" style="color:#333">\u63A5\u6536\u65B0\u90AE\u4EF6\uFF0C\u5E76\u5728QQ\u63D0\u9192</label>', '</div>', '<div style="padding:5px 0 3px">', '<input type="radio" name="qqnotify" class="cb" value="notifyno" $@$if(!$bReject$&&!$bNotify$)$@$checked="true"$@$endif$@$ id="noqqnotify"/>', '<label for="noqqnotify" style="color:#333">\u63A5\u6536\u65B0\u90AE\u4EF6\uFF0C\u4E0D\u5728QQ\u63D0\u9192</label>', '</div>', '<div style="padding:5px 0 3px;">', '<input type="radio" name="qqnotify" class="cb" value="reject" $@$if($bReject$)$@$checked="true"$@$endif$@$ id="reject"/>', '<label for="reject" style="color:#333">\u62D2\u6536\u90AE\u4EF6</label>', '</div>', '</div>', '</div>', '<div style="position:relative;border-top:1px solid #ccc;padding-top:8px;$@$if($bReject$)$@$display:none;$@$endif$@$;" class="clear" id="setnewdiv">', '<p style="margin:0;">\u5BF9\u6B64\u7FA4\u5DF2\u8BFB\u90AE\u4EF6:</p>', '<div style="margin-left:8px;">', '<div style="padding:5px 0 3px">', '<input type="radio" name="unread" class="cb" value="setnewyes" $@$if($bNewreply$)$@$checked="true"$@$endif$@$ id="setunread"/>', '<label for="setunread" style="color:#333">\u6709\u65B0\u56DE\u590D\u65F6\uFF0C\u6807\u4E3A\u672A\u8BFB</label>', '</div>', '<div style="padding:5px 0 3px">', '<input type="radio" name="unread" class="cb" value="setnewno" $@$if(!$bNewreply$)$@$checked="true"$@$endif$@$ id="setread"/>', '<label for="setread" style="color:#333">\u6709\u65B0\u56DE\u590D\u65F6\uFF0C\u4FDD\u6301\u5DF2\u8BFB</label>', '</div>', '</div>', '</div>', '<div style="padding:10px 0;overflow:hidden;" class="clear">', '<a class="btn_blue btn_space" id="save">\u4FDD\u5B58\u66F4\u6539</a>', '<a class="btn_gray" href="javascript:;" id="cancel">\u53D6\u6D88</a>', '</div>', '</div>']),
        _TEMPLATE = {
            _REPLY_DLG: T(['<div class="dialog_feedback">', '<span class="dialog_icon icon_info_b"></span>', '<div class="dialog_f_c">', '<div class="dialog_f_t">\u662F\u5426\u8981\u56DE\u590D\u5168\u90E8\u6536\u4EF6\u4EBA\uFF1F</div>', '<div class="dialog_f_d"> \u60A8\u662F\u5BC6\u9001\u7684\u6536\u4EF6\u4EBA\uFF0C\u5176\u4ED6\u6536\u4EF6\u4EBA\u4E0D\u77E5\u9053\u60A8\u6536\u5230\u4E86\u6B64\u90AE\u4EF6\uFF01</div>', '</div>', '</div>']),
            _REPLY_DLG_FOOT: T(["<input type=button id='replyall' class='btn wd4' value='\u56DE\u590D\u5168\u90E8'>", "<input type=button class='btn wd4' value='\u56DE\u590D\u53D1\u4EF6\u4EBA' id='reply'>", "<input type=button class='btn wd4' value='\u53D6\u6D88' id='cancel'>"]),
            _REFERPART: TE(['<div><br></div><div><br></div>', '<div style="font-size: 12px;font-family: Arial Narrow;padding:2px 0 2px 0;">', '------------------&nbsp;$REFER$&nbsp;------------------', '</div>', '<div style="font-size: 12px;background:#efefef;padding:8px;">', '<div><b>$FROM$</b> ', _ADDR_, '</div>', '<div><b>$DATE$</b> $date$</div>', '<div><b>$TO$</b> $@$for($to$)$@$', _ADDR_, '$@$endfor$@$</div>', '$@$if($cc.length$)$@$', '<div><b >$CC$</b> $@$for($cc$)$@$', _ADDR_, '$@$endfor$@$</div>', '$@$endif$@$', '<div><b>$SUBJECT$</b> $subject$</div>', '</div><div><br></div>', '$orgcontent$'])
        }, _LANG = {
            _ZH_CN: {
                REFER: "\u539F\u59CB\u90AE\u4EF6",
                FROM: "\u53D1\u4EF6\u4EBA:",
                ...
                CC: "\u6284\u9001:",
                SUBJECT: "\u4E3B\u9898:"
            }, _EN_US: {REFER: "Original", FROM: "From:", DATE: "Date:", TO: "To:", CC: "Cc:", SUBJECT: "Subject:"}
        },
        _BW_LIST = T("/cgi-bin/config_blackwhitelist?sid=$sid$&act=whitelist&Fun=submit&pagefrom=readmail&group=$fromaddr$&sloc=readmail_yellow_tip"),
        _SPAM_REPORT = T("/cgi-bin/report_cgi?r_type=$rtype$&r_result=$rresult$&r_msg=$rmsg$");
    ...
    function _createForm(_aoConfig, _aoInputData) {
        ...
        insertHTML(_aoConfig.oWin.document.body, "beforeEnd", _FORM_TMP.replace(extend({
            sFormId: _sFormId,
            sTarget: "actionFrame",
            sMethod: "POST"
        }, _aoConfig)));
    ...
    function _showQBTip(_aoTarget) {
        var _oPosInfo = calcPos(_aoTarget), _oMainFrmPos = calcPos(S("mainFrame", getTop())),
            _oMenu = QMMenu("readmail_qb_push_tip");
        if (_oMenu) {
            _oMenu.show();
        }
        else {
            new QMMenu({
                sId: "readmail_qb_push_tip",
                sClassName: "qmpanel_shadow",
                nX: _oPosInfo[3] - 160,
                nY: _oPosInfo[2],
                nWidth: 258,
                bAutoClose: true,
                oEmbedWin: this._moWin,
                oItems: [{
                    nHeight: 110,
                    sItemValue: ['<div style="line-height:20px;padding:15px 0 15px;white-space:normal;">', '\u5EFA\u8BAE\u5B89\u88C5QQ\u6D4F\u89C8\u5668\uFF0C\u53EF\u6709\u6548\u62E6\u622A\u9493\u9C7C\u548C<br/>\u6B3A\u8BC8\u7F51\u7AD9\uFF0C\u9632\u6B62\u5E10\u53F7\u4FE1\u606F\u88AB\u76D7\u3002', '</div>', '<div class="txt_right" style="line-height:30px;">', '<a href="javascript:;" class="btn_blue" id="setup_btn">\u7ACB\u5373\u5B89\u88C5</a>', '</div>'].join('')
                }],
                onload: function () {
                    var _oSelf = this;
    ...
    function _showBccTip(_aoTarget) {
        ...
        new QMMenu({
            ...
            oItems: [{
                nHeight: 50,
                sItemValue: ['<div style="line-height:20px;padding:15px 1px 15px;white-space:normal; text-align:center;">', attr(_aoTarget, 'data-bcc'), '</div>'].join('')
            }]
        });
    }
    ...
    var _qmBaseDM = _oClassHome.qmBaseDM = _oClassHome._qmBaseDM = inherit("_qmBaseDM", Object, function (_aoSuper) {
        return {
            ...
            _handle: function (_aoModDom, _aoEvent, _asItem) {
                var _oSelf = this, _oItDom = getEventTarget(_aoEvent), _oDoc = _oItDom.ownerDocument,
                    _sModuleName = _oSelf._msModuleName, _sAttr, _sMethod, _oTarget;
            ...
            confirmOpen: function (_aoCfg, _afCallback) {
                getTop().confirmBox({
                    title: "\u8B66\u544A",
                    msg: ['<div class="dialog_f_t" style="margin-top: 2px;font-weight: normal;"> ' + _aoCfg.msg + '</div>'].join(""),
                    confirmBtnTxt: _aoCfg.btnTxt,
                    cancelBtnTxt: '\u53D6\u6D88',
                    onshow: function () {
            ...
            processQQMailAppH5CardForWebmail: function () {
                ...
                var deskDom = _oTop.finds("table.greetingCard_desktop_table div", containerDom),
                    firstDiv = deskDom && deskDom[0];
            ...
            checkOnlineDocStarStatus: function (_asIndex) {
                var _oSelf = this, _oTop = getTop(),
                    _sUrl = "/cgi-bin/docedit_read?t=online_doc.json&func=checkstar&sid=" + _oTop.getSid() + "&key=";
                var _oOnlineDocAttachDoms = _oTop.finds("div.att_bt a[ck='starOnlineDoc']", _oSelf.getMainContainer() || _oSelf._moWin),
                    _oOnlineDocAttachs = {}, _oOnlineDocAttachIds = [], _oOnlineDocAttachId;
                ...
                QMAjax.send(_sUrl, {
                    method: "GET", onload: function (_abIsOk, _asResult) {
                        if (_abIsOk) {
                            ...
                            if (_oResult.ret == 0) {
                                ...
                                _oTop.E(_oList, function (_aoAttach) {
                                    if (_aoAttach.bHasDeleted) {
                                        var _oItemContainerDom = _oTop.parents("div.att_bt", _oOnlineDocAttachs[_aoAttach.sFileId])[0],
                                            _oFileSizeDom = _oTop.finds("span.graytext", _oItemContainerDom)[0],
                                            _oImgDom = _oTop.finds("a[ck='editOnlineDoc']", _oItemContainerDom)[0];
            ...
            checkPoison: function (_asMailId) {
                ...
                QMAjax.send(T('/cgi-bin/do?action=checkvirus&sid=$sid$&mailid=$mailid$&t=do.json&cacheage=7200&ver=$ver$').replace({
                    sid: getSid(),
                    mailid: _sMailId,
                    ver: getTop().CommVer.get("checkPoison")
                }), {
                    method: "GET", onload: function (_abIsOk, _asResult) {
                        ...
                        if (_abIsOk && _sText.indexOf("(") == 0) {
                            ...
                            for (var id in _oResJson) {
                                ...
                                if (_oResJson[id].isSafe == "0" || _oResJson[id].isSpam == "1") {
                                    ...
                                    var _oParentDom = _oAttachDom && _oAttachDom.parentNode && _oAttachDom.parentNode.parentNode,
                                        _oDownDom = _oTop.finds("div.down_big", _oParentDom),
                                        _oImgPreview = _oTop.finds("a", _oParentDom.parentNode);
                                    ...
                                    var _oDown = _oTop.S("attach_tools_download_" + id, _oSelf._moWin),
                                        _oPreview = _oTop.S("attach_tools_preview_" + id, _oSelf._moWin);
        ...
        _MOD_STATE: "_module_state_",
        _MOD_ATTR: "module",
        _CON_ATTR: "context",
        _AUX_ATTR: "aux",
        _oEvtMap: {mor: "mouseover", mot: "mouseout", ck: "click", md: "mousedown", mu: "mouseup", dck: "dblclick"}
    });
    _oClassHome.qmReadMail = inherit("qmReadMail", _oClassHome._qmBaseDM, function (_aoSuper) {
        return {
            _ready: function () {
                ...
                QMAjax.send(T('/cgi-bin/readmail?sid=$sid$&rank=$rank$&mailid=$mailid$&t=readsubmail&mode=fake&s=r2&base=$base$&pf=$pf$&folderid=$folderid$&folderkey=$folderkey$').replace({
                    sid: getSid(),
                    mailid: _oSelf.getMailId(),
                    pf: rdVer.isPre(_oConfig.folderid) ? 1 : 0,
                    folderid: _oConfig.folderid,
                    folderkey: _oConfig.folderkey,
                    rank: _oCache.getData(_oSelf.getMailId()),
                ...
                var _oSelf = this, _oConfig = _oSelf._moConfig, sMailId = _oSelf.getMailId(),
                    _oCacheData = QMMailCache.getData(sMailId);
                if ((_oConfig.bNewMail == 1) && (!_oCacheData || _oCacheData.bUnread === true)) {
                    folderOpt({
                        bNewMail: _oConfig.bNewMail,
                        sFolderId: _oConfig.folderid,
                        sMailId: sMailId,
                        oMatchTag: _oConfig.oMatchTag,
                        oSysTag: _oConfig.oSysTag,
                        bStar: _oConfig.bStar,
                        bAddrvip: _oConfig.bAddrvip
                    });
                }
                else {
                ...
                _oWin.setMoreOperation = function (_asId1, _asId2) {
                    ...
                    return T("/cgi-bin/readtemplate?sid=$sid$&t=video_ref.smil&vsrc=$vsrc$").replace({
                        sid: getSid(),
                        vsrc: encodeURIComponent(_oWin.sFlvPlayUrl.substr(1))
                    });
                };
                ...
                var _oSelf = this, _sNext, _oPreNextTop = _oSelf.S(_ID._sNextMailTop),
                    _oPreNextBt = _oSelf.S(_ID._sNextMailBt), _oNextNew = _oSelf.S(_ID._sNextNewMail);
                    ...
                    case "addremind":
                        ...
                        if (_oLink && _oLink[0]) {
                            if (_aoItem.sId == "addremind") {
                                ...
                                var _oMap = getUrlParams(S("remind_delete", document).href || location),
                                    _oFrm = document.remind_frm;
                ...
                var _oSelf = this, _oModule, _oMailInfo = _oSelf._moMailInfo,
                    _oContext = extend({}, _oSelf._moContext, _aoSubMod, {oMailInstance: _oSelf}),
                ...
                switch (_oContext.sModuleName) {
                    case "qmRemark":
                        ...
                        break;
                    case "qmMoreOptSel":
                        _oModule = _oSelf._moMoreOptSel = new _oClassHome.qmMoreOptSel({
                            oMoreOpt: _oConfig.oMoreOpt, fOnSelect: function (_asItemId) {
                                _oSelf._onMoreOptSel(_asItemId);
                            }
                        }, _oContext);
                        break;
                    case "qmQReply":
                        _oModule = _oSelf._moQReply = new _oClassHome.qmQReply(_oConfig, _oMailInfo, {
                            fCheckBcc: function () {
                    ...
                    case "qmSenderInfo":
                        ...
                        break;
                    case "qmPlayerParser":
                        _oModule = new _oClassHome.qmPlayerParser({
                            oContentDom: _oSelf.S(_ID._sContentDiv),
                            bManuPlay: _oConfig.bMusicManuPlay
                        }, _oContext);
                        break;
                    case _sQMTRANSLATE:
                ...
                _oSelf._moWin.document.body.oncopy = function (_aoEvent) {
                    if (!_bLock) {
                        var _oTarget = _oTop.getEventTarget(_aoEvent), _sCopyContent = "", _nMaxLen = 256, _oItems = [[{
                            sId: "",
                            sItemValue: '<span class="icon_caution_s"></span>\u8BE5\u5730\u5740\u5B58\u5728\u4E0D\u826F\u5185\u5BB9\uFF0C\u5EFA\u8BAE\u505C\u6B62\u8BBF\u95EE',
                            bDisSelect: true
                        }], [{
                            sId: "",
                            sItemValue: '<span class="icon_info_s"></span>\u8BE5\u5730\u5740\u5B89\u5168\u6027\u65E0\u6CD5\u786E\u5B9A\uFF0C\u8BF7\u8C28\u614E\u6253\u5F00',
                            bDisSelect: true
                        }], ""];
                        ...
                        if (_sCopyContent != "") {
                            ...
                            _bLock = true;
                            QMAjax.send(T("/cgi-bin/mail_spam?sid=$sid$&stat=getcopycontent&content=$content$&t=getcopycontent&action=copy_link").replace({
                                sid: getSid(),
                                content: _oTop.encodeURI(_sCopyContent)
                            }), {
                                method: "POST", onload: function (_abIsOk, _asParam) {
                                    ...
                                    if (_abIsOk) {
                                        if (_asParam != "2") {
                                            new QMMenu({
                                                oEmbedWin: _oSelf._moWin,
                                                sClassName: (_asParam == 0 ? "tips_maliciousLink" : "tips_unknowLink"),
                                                sid: "report",
                                                oItems: _oItems[_asParam],
                                                nWidth: 237,
                                                nX: _nX,
                                                nY: _nY + _nHeight + 5,
                                                bAutoClose: false
                ...
                var _oTop = getTop(), _oSelf = this, _oWin = _oSelf._moWin, _oMailInfo = _oSelf._moMailInfo,
                    _oConfig = _oSelf._moConfig, _oContentDiv = _oSelf.S(_ID._sContentDiv),
                    _oFolder = S("folder_" + _oConfig.folderid, _oTop), _sBgUrl = _oWin.document.body.background;
                if (!_oContentDiv) {
                    return;
                }
                _oSelf._rInsPageEnd();
                ...
                if (_oFolder && _oFolder.parentNode.parentNode.id == "folder_pop_td") {
                    new QMSender({
                        oWin: _oWin,
                        nCurFolderId: _oConfig.folderid,
                        sWidth: 210,
                        sCurSaveFrom: _oConfig.saveFrom
                    });
                }
                ...
                function _fCheckImgSize(_nWidth, _nHeight, _aoImgNode) {
                    ...
                    if (_nWidth > _nCurWidth || _nHeight > _nCurHeight) {
                        _oTop.ossLogCustom('delay', 'all', 'attImgtobig', {
                            src: _aoImgNode.src,
                            max: {w: _nWidth, h: _nHeight},
                            cur: {w: _nCurHeight, h: _nCurHeight}
                        });
                ...
                if (_oSTSpan) {
                    ...
                    getTop().addEvent(_oEditSpan, "click", function () {
                        initTimeSendDlg({
                            sendtimeyear: _oTop.S("sendtimeyear", _oTop.getMainWin()),
                            sendtimemonth: _oTop.S("sendtimemonth", _oTop.getMainWin()),
                            sendtimeday: _oTop.S("sendtimeday", _oTop.getMainWin()),
                            sendtimehour: _oTop.S("sendtimehour", _oTop.getMainWin()),
                            sendtimemin: _oTop.S("sendtimemin", _oTop.getMainWin()),
                            confirmbtn: "\u786E\u5B9A",
                            title: "\u4FEE\u6539\u65F6\u95F4",
                            timetips: ""
                        }, function () {
                            QMAjax.send(T(["/cgi-bin/mail_mgr?mailaction=mdy_sendtime&mailid=$mailid$&ef=js&resp_charset=UTF8", "&sendtimeyear=$sendtimeyear$", "&sendtimemonth=$sendtimemonth$", "&sendtimeday=$sendtimeday$", "&sendtimehour=$sendtimehour$", "&sendtimemin=$sendtimemin$"]).replace({
                                mailid: _oSelf.getMailId(),
                                sendtimeyear: _oTop.S("sendtimeyear", _oTop.getMainWin()).value,
                                sendtimemonth: _oTop.S("sendtimemonth", _oTop.getMainWin()).value,
                                sendtimeday: _oTop.S("sendtimeday", _oTop.getMainWin()).value,
                                sendtimehour: _oTop.S("sendtimehour", _oTop.getMainWin()).value,
                                sendtimemin: _oTop.S("sendtimemin", _oTop.getMainWin()).value
                            }), {
                                method: "POST", onload: function (_abIsOk, _asParam) {
                                    if (_abIsOk) {
                ...
                if (_sCode) {
                    QMAjax.send(T("/cgi-bin/readmail?sid=$sid$&mailid=$mailid$&action=decryptmail&t=success&secmailcode=$code$&ef=js&resp_charset=UTF8").replace({
                        ...
                        method: "GET", onload: function (_abIsOk, _asParam) {
                            if (_abIsOk) {
                                var _oRet = evalValue(_asParam);
                                if (_oRet.errcode == "0") {
                ...
                var _oSelf = this, _oWin = _oSelf._moWin, _oMailInfo = _oSelf._moMailInfo, _oTop = getTop(),
                    _oUnRead = _oTop.S("submailCnt", _oWin), _oTotal = _oTop.S("submailCntAll", _oWin),
                    _nUnReadCnt = _oTop.finds(".mailunread", _oWin).length,
                    _nReadCnt = _oTop.finds(".mailread", _oWin).length, _nTotal = _nUnReadCnt + _nReadCnt;
                if (_abNoSet !== true) {
                ...
                var _oSelf = this, _oConfig = _oSelf._moConfig, _oMailInfo = _oSelf._moMailInfo,
                    _oFrom = _oSelf.getFromInfo_(), _oStar = _oSelf.S(_ID._sImgStar), _oTopBtn = _oSelf.S(_ID._sTopBtn),
                    _sSysTag = _oConfig.oSysTag || "", _oTagArr = [], _oSysTag = {};
                if (_sSysTag != "") {
                ...
                return {
                    oWin: _oSelf._moWin,
                    sFid: _oConfig.folderid,
                    bML: false,
                    oMail: [{
                        sMid: _oSelf.getMailId(),
                        bSys: _oConfig.bSys,
                        bAd: _oConfig.bAd,
                        bUnr: false,
                        bSubUnr: false,
                        bStar: hasClass(_oStar, 'qm_ico_flagon'),
                        bTop: hasClass(_oTopBtn, 'qm_ico_topon'),
                        bTms: false,
                        oSysTag: _oSysTag || {},
                        oTagIds: _oConfig.oMatchTag,
                        sSName: _oFrom.name,
                        sSEmail: _oFrom.addr,
                        oStar: _oStar,
                        oTopBtn: _oTopBtn,
                        oTCont: _oSelf.S(_ID._sTagContainer)
                    }]
                };
                ...
                var _oSelf = this, _oSplashDom = _oSelf.S(_ID._sAttachment + (_oSelf.attr(_aoTarget, "seq") || "")),
                    _oDocElem;
                ...
                var _oSelf = this;
                starMail(null, extend(_oSelf.getCBInfo(), {
                    oncomplete: function (_aoCBInfog, _abIsSet) {
                        _oSelf.clearCache();
                        var _oStarStatus = _oSelf.S(_ID._sStarStatus), _oMailtopStatus = _oSelf.S(_ID._sMailtopStatus),
                            _oStarTopStatus = _oSelf.S(_ID._sStarTopStatus);
                        if (_abIsSet) {
                ...
                var _oSelf = this;
                topMail(null, extend(_oSelf.getCBInfo(), {
                    oncomplete: function (_aoCBInfog, _abIsSet) {
                        _oSelf.clearCache();
                        var _oStarStatus = _oSelf.S(_ID._sStarStatus), _oMailtopStatus = _oSelf.S(_ID._sMailtopStatus),
                            _oStarTopStatus = _oSelf.S(_ID._sStarTopStatus);
                ...
                var _oSelf = this, _oAttachItem = parents("div.attachitem", _aoTarget)[0],
                    _oPreviewItem = finds("a[ck='previewAttach2']", _oAttachItem)[0];
                ...
                var _oSelf = this,
                    _oTarget = S(["AttachIconA", _asMailId, (_oSelf.attr(_aoTarget, "idx") || 0)].join(""), _oSelf._moWin);
                ...
                function _previewAttach2() {
                    var _fIsTimeout = function (_aoTarget) {
                        ...
                        var _oRe = {}, _oTarget = S(["AttachIconA", _asMailId, _asRank].join(""), _oSelf._moWin),
                            _nCnt = +_asRank;
                        ...
                        var _sDown = _oTarget.getAttribute('down'),
                            _oTemp = S(["AttachIconA", _asMailId, (_nCnt - 1)].join(""), _oSelf._moWin);
                        ...
                        E({
                            ...
                            sUrl: "url",
                            sType: "viewmode",
                            sParse2OnlineDocUrl: "sParse2OnlineDocUrl",
                            sBytesize: "filebyte",
                            sThumb: "iconurl",
                            sFileIdx: "idx",
                            sAttid: "attid",
                        ...
                        var _oCallBacks = {
                            fNext: function (_aoCurFileInfo) {
                                ...
                                if (_aoCurFileInfo.bBigAttach) {
                                    goNewWin(_BIG_ATT_FW_SEND.replace({
                                        sid: getSid(),
                                        attachid: encodeURIComponent(_aoCurFileInfo.sAttid),
                                        urifilename: _aoCurFileInfo.sUriFileName,
                                        filesize: _aoCurFileInfo.sBytesize,
                                        exptime: _aoCurFileInfo.sExptime,
                                        k: _fGetUrlParam(_aoCurFileInfo.sUrl, "k"),
                                        code: _fGetUrlParam(_aoCurFileInfo.sUrl, "code"),
                                        fid: _fGetUrlParam(_aoCurFileInfo.sUrl, "fid")
                                    }));
                                }
                                else {
                                    goNewWin(_ATT_FW_SEND.replace({
                                        sid: getSid(),
                                        mailid: _asMailId,
                                        attachid: encodeURIComponent(_aoCurFileInfo.sAttid),
                                        attachname: _aoCurFileInfo.sAttid,
                                        fileextenal: _aoCurFileInfo.sSuffix,
                                        filebyte: _aoCurFileInfo.sBytesize
                                    }));
                                }
                            }
                        };
                ...
                var _oSelf = this, _sReadMail = T(_READMAIL_).replace({sid: getSid(), mailid: _asMailId}),
                    _sUrl = T("$url$&nocheckframe=true&t=attachpreviewer&select=$select$&selectfile=$selectfile$&seq=$seq$").replace({
                        url: _sReadMail,
                        select: _oSelf.attr(_aoTarget, "select"),
                        seq: _oSelf.attr(_aoTarget, "seq") || "",
                        selectfile: _oSelf.attr(_aoTarget, "selectfile") || ""
                    });
                ...
                var _oSelf = this, _sFileId = _oTop.attr(_aoTarget, "onlineDocKey"),
                    _sReadUrl = ["/cgi-bin/docedit_read?t=online_doc.html&key=", _sFileId].join(""),
                    _sNewWinEditUrl = [_oTop.location.protocol, "//", _oTop.location.hostname, "/cgi-bin/frame_html?t=newwin_frame&sid=", _oTop.getSid(), "&url=", encodeURIComponent(_sReadUrl)].join("");
                _oTop.open(_sNewWinEditUrl, "_blank");
            }, exportOnlineDoc: function (_aoTarget, _asMailId, _aoEvent) {
                var _sFileId = _oTop.attr(_aoTarget, "onlineDocKey"), _sFileName = _oTop.attr(_aoTarget, "filename"),
                    _sExportDoc = ["/cgi-bin/docedit_redirect?func=export&sid=", _oTop.getSid(), "&key=", _sFileId, "&name=", encodeURIComponent(_sFileName)].join("");
                window.open(_sExportDoc, "actionFrame");
            }, viewOnlineDoc: function (_aoTarget, _asMailId, _aoEvent) {
                var _oSelf = this, _sFileId = _oTop.attr(_aoTarget, "onlineDocKey"),
                    _sViewUrl = ["/cgi-bin/docedit_list?t=online_doc&page=0&pagesize=40&sid=", _oTop.getSid(), "&viewkey=", _sFileId].join("");
                ...
                var _oSelf = this, _spans = GelTags("span", _aoTarget.parentNode.parentNode),
                    _span0 = _spans.length == 0 ? {} : _spans[0], _oContainer = _oSelf.S(_ID._sMp3PlayerContainer);
                if (_span0.getAttribute("player")) {
                    ...
                    audioPlay({
                        ...
                        url: _span0.getAttribute("player"),
                        title: _span0.innerHTML,
                        autoplay: true,
                        global: true
                    });
                    return true;
                ...
                new QMDialog({
                    sId: "checkdownload",
                    sTitle: "\u4E0B\u8F7D\u63D0\u901F",
                    sUrl: T("/cgi-bin/readtemplate?sid=$sid$&t=readmail_checkdownload").replace({sid: getSid()}),
                    nWidth: 461,
                    nHeight: 175
                });
                preventDefault(_aoEvent);
                ...
                if (_oSelf._moConfig.bBccToMe) {
                    ...
                    new QMDialog({
                        sId: "reply_dlg",
                        sTitle: "\u56DE\u590D\u63D0\u793A:",
                        sBodyHtml: _TEMPLATE._REPLY_DLG.toString(),
                        sFootHtml: _TEMPLATE._REPLY_DLG_FOOT.toString(),
                        onshow: function () {
                ...
                if (_oConfig.xqqstyle == "6") {
                    _createForm({oWin: _oWin, sAction: "/cgi-bin/mail_mgr"}, {
                        mailid: _asMailId,
                        mailaction: "autodel",
                        reporttype: _attr(_aoTarget, "flag"),
                        s: "autodel"
                    }).submit();
                }
            }, mailRecall: function (_aoTarget, _asMailId) {
                var _oTop = getTop(), _sLocCnt = attr(_aoTarget, "loccnt"), _sMid = attr(_aoTarget, "mid"),
                    _sMDate = attr(_aoTarget, "mdata"), _sOdd = attr(_aoTarget, "odd"),
                    _bIsSlefSend = (_oTop.g_encryptuin == attr(_aoTarget, "u")), _sActId = 0;
                if (_sLocCnt > 0 && _sOdd != "0") {
                    _sActId = 1;

                    function recallCheck() {
                        _oTop.recallConfirm(_sMid, _aoTarget, {
                            oninit: function (_aoDom) {
                                this._moDom = _aoDom;
                    ...
                    QMAjax.send(T("/cgi-bin/send_status?t=send_status.json&s=mailrecallv2&messageid=$msgid$&time=$time$&sid=$sid$&ef=js&mailid=$mailid$&checkonly=1").replace({
                        sid: getSid(),
                        msgid: _sMid,
                        time: attr(_aoTarget, "time"),
                        mailid: _asMailId
                    }), {
                        method: "GET", timeout: 3000, onload: function (_abIsOk, _avParam) {
                            if (_abIsOk) {
                    ...
                    msgBox(T(['<b class="b_size">$t$</b>', '<div style="overflow:hidden;margin-top:5px;">', '$m$', '</div>']).replace({
                        t: _sTitle,
                        m: _sMsg
                    }), "dialog");
                }
                ...
                _oConf = {
                    ...
                    oItems: [{
                        nHeight: "auto",
                        bDisSelect: true,
                        sItemValue: _MARK_READ_STATUS_T.replace({displayon: _sDisplayOn, displayoff: _sDisplayOff})
                    }],
                    onshow: function () {
                        var _oSwitchOn = finds('#' + _sId + '_QMMenu_switch_on div a', _oEmbedWin)[0],
                            _oSwitchOff = finds('#' + _sId + '_QMMenu_switch_off div a', _oEmbedWin)[0],
                            _oSwitchOnDiv = S(_sId + "_QMMenu_switch_on", _oEmbedWin),
                            _oSwitchOffDiv = S(_sId + "_QMMenu_switch_off", _oEmbedWin), _oMenu = this;

                        function _getSwitchCallback(_abSwitch) {
                            var _sUrl = T(['/cgi-bin/config_blackwhitelist?', 'sid=$sid$&Fun=addad', '&addr=$addr$:$status$', '&from=self']).replace({
                                sid: getSid(),
                                addr: _sAddr,
                                status: (_abSwitch ? '0' : '1')
                            });
                            return function () {
                                _oMenu.close();
                                QMAjax.send(_sUrl, {
                                    onload: function (_abFlag, _asParam) {
                                        if (_abFlag) {
                                            _oSelf.clearCache();
                ...
                var _oSelf = this, _oWin = _oSelf._moWin, _sTarget = "_blank", _sOpt = _oSelf.attr(_aoTarget, "opt"),
                    _sReadMail = T(_READMAIL_).replace({sid: getSid(), mailid: _asMailId});
                switch (_sOpt) {
                    case "remind":
                        _sReadMail = T("/cgi-bin/readtemplate?t=calendar&sid=$sid$&from=readmail&cmd=moncal,,new,$subject$,mail,$mailid$").replace({
                            sid: getSid(),
                            mailid: _asMailId,
                            subject: encodeURIComponent(_oSelf._moMailInfo.subject || "(\u65E0\u4E3B\u9898)")
                        });
                        _sTarget = "mainFrame";
                        rdVer(_asMailId, 1);
                        break;
                    ...
                    case "addc":
                        ...
                        QMAjax.send(T('/cgi-bin/addr_addedit?sid=$sid$&category=&isqqgroup=&addr_errorflag=&OperType=ADD&Type=&GROUPLIST=&s=addfrommail&UseFreq=&USERNAME=$username$&USEREMAILNAME=$email$&USEREMAILNAME=&MOBILEPHONE=&PHS=&NICKNAME=&BRITHDAY=&MAILBOX_BACK=&STREET=&COMPANY=&NOTES=&list_index=&ef=js&inputcharset=utf8').replace({
                            sid: getSid(),
                            username: encodeURIComponent(_sUsername),
                            email: encodeURIComponent(_sEmail)
                        }), {
                            method: "POST",
                            headers: {"If-Modified-Since": "0", "Cache-Control": "no-cache, max-age=0"},
                            onload: function (_abIsOk, _asParam) {
                ...
                var _oSelf = this, _bIsAd = _oSelf.getCBInfo().oMail[0].bAd,
                    _nIsTuan = _oSelf.attr(_aoTarget, "istuan") || 0, _oTop = getTop();
                _oTop.loadingBox({
                    model: "\u53CD\u5783\u573E", js: ["$js_path$qmantispam264eed.js"], oncheck: function () {
                        return !!getTop().QMAntiSpam;
                    }, onload: function () {
                        var _oSpam = new QMAntiSpam.qmExbookEmlMgr({
                            sMailId: _asMailId,
                            from: _oSelf.getMailInfo().from
                        });
                ...
                getTop().QMMLCache.upVer();
                _bRet = rmMail(_oSelf.attr(_aoTarget, "opt") || 0, extend(_oSelf.getCBInfo(), {
                    oncomplete: function (_aoCBInfo, _aoResult) {
                ...
                var _oSelf = this, _oWin = _oSelf._moWin, _oConfig = _oSelf._moConfig,
                    _sOpt = _oSelf.attr(_aoTarget, "opt"), _sPlusContent = _oSelf.getEditorContent(),
                    _sFolderId = _oConfig.folderid, _sCpsMailUrl = _READMAIL_CPS_URL.replace({
                        sid: getSid(),
                        t: _oSelf.attr(_aoTarget, "t"),
                        s: _sOpt,
                        mailid: _asMailId,
                        disptype: _oConfig.disptype == "text" ? "" : "html"
                    });
                ...
                function opt(_asOpt) {
                    ...
                    if (_aoEvent && _aoEvent.shiftKey) {
                        ...
                        _createForm({
                            ...
                            sTarget: "mainFrame",
                            sMethod: "POST",
                            sAction: _sCpsMailUrl
                        }, {pluscontent: htmlEncode(_sPlusContent)}).submit();
                    }
                    else {
                        var _sUrl = _sCpsMailUrl + _oMore.join("");
                        if (_oSelf.dealCustomUI('optMail', {
                                sAction: 'closePage',
                                sType: _asOpt,
                                sUrl: _sUrl
                            }) !== false) {
                ...
                switch (_sOpt) {
                    ...
                    case "forward":
                        if (_sOpt == "forward") {
                            ...
                            if (_sKeyWord) {
                                var _nIndex = _sKeyWord.lastIndexOf(".") || _sKeyWord.length,
                                    _sFileName = _sKeyWord.substring(0, _nIndex),
                                    _sSuffix = _sKeyWord.substring(_nIndex);
                                ...
                                _oTop.confirmBox({
                                    title: "\u63D0\u793A",
                                    nWidth: 440,
                                    msg: ['<div class="dialog_f_t" style="font-weight: normal;word-break: break-all;">\u8D85\u5927\u9644\u4EF6 "', _sFileDesc, '" ', _sMore, '\uFF0C\u8F6C\u53D1\u5C06\u81EA\u52A8\u5220\u9664\u3002</div>'].join(""),
                                    confirmBtnTxt: '\u7EE7\u7EED\u8F6C\u53D1',
                                    onreturn: function (_abIsOk) {
                                        if (_abIsOk) {
                                            opt(_sOpt);
                ...
                var _oCnChar = _sContentText.match(/[\u2100-\uFFFF]/g) || [],
                    _oUsChar = _sContentText.match(/[a-zA-Z]/g) || [],
                    _oUsWord = _sContentText.match(/[a-zA-Z]+[\u2100-\uFFFF\s]/g, "") || [];
                ...
                var _oSelf = this, _oTop = getTop(), _oWin = _oSelf._moWin, _oPanel = _oTop.S("showcalpanel", _oWin),
                    _sDate = _aoTarget.innerHTML,
                    _oMatch = /((?!0000)[0-9]{4}\u5E74)?((0?[1-9]|1[0-2])\u6708(0?[1-9]|1[0-9]|2[0-8])\u65E5|(0?[13-9]|1[0-2])\u6708(29|30)\u65E5|(0?[13578]|1[02])\u670831\u65E5)/g,
                    _sTmpl = T(['<span id="showcalpanel" class="showcalpanel" onmouseover="getTop().attr(this.parentNode, \'isout\', 0);QMReadMail.setStatus(1)" onmouseout="QMReadMail.setStatus(0);QMReadMail.hideCalEvent()">', '<a onclick="QMReadMail.goCal(\'$date$\', \'$mailId$\')" class="add2calendar"><span class="ico_add2cal"></span>\u6DFB\u52A0\u5230\u65E5\u5386</a>', '</span>']),
                    _sTime = _oTop.attr(_aoTarget, "times") || "";
                ...
                if (/^[\d\-\:\s]+$/.test(_sDate) && !isNaN((new Date(_sDate.replace(/-/g, '/')).getTime()))) {
                    ...
                    _oTop.insertHTML(_aoTarget, "beforeEnd", _sTmpl.replace({
                        date: _sDate,
                        images_path: _oTop.getPath("images"),
                        mailId: _asMailId
                    }));
                }
                ...
                var _oSelf = this, _sMailId = _asMailId || _oSelf.getMailId(), _sFrom = 'mail', _oTop = getTop(),
                    _sSubject = _asSubject || (_oSelf._moMailInfo.subject || _oTop.tmpSubject);
                ...
                var _sCalUrl = T("/cgi-bin/readtemplate?t=calendar&sid=$sid$&from=readmail&cmd=moncal,$date$,new,$subject$,$from$,$mailId$,&loc=readmail,calendar,,,0,1").replace({
                    sid: _oTop.getSid(),
                    date: _asDate,
                    subject: encodeURIComponent(_sSubject || "(\u65E0\u4E3B\u9898)"),
                    mailId: _sMailId,
                    from: _sFrom
                });
    ...
    _oClassHome.qmReminder = inherit("qmReminder", _oClassHome._qmBaseDM, function (_aoSuper) {
        return ({
            _ready: function () {
                ...
                QMAjax.send(T("/cgi-bin/read_reminder?t=read_reminder.json&linkid=mailid:$mailid$&sid=$sid$&rettype=calendar").replace({
                    sid: getSid(),
                    mailid: _sMailId
                }), {
                    method: "GET", onload: function (_abIsOk, _asParam) {
                        if (_abIsOk) {
                            ...
                            if (_oJson && _oJson.id) {
                                ...
                                _oContainer.innerHTML = TE("<span class='qm_ico_calendar'></span><span class='addrtitle'>\u4E8B\u4EF6\u65F6\u95F4\uFF1A$starttime$</span>&nbsp;<a href='/cgi-bin/readtemplate?sid=$sid$&t=calendar&cmd=moncal,$starttime$,view,$id$'>\u67E5\u770B\u65E5\u5386</a>").replace({
                                    starttime: _oJson.starttime,
                                    allday: _oJson.allday,
                                    sid: getSid(),
                                    id: _oJson.id
                                });
                                show(_oContainer, true);
    ...
    _oClassHome.qmQReply = inherit("qmQReply", _oClassHome._qmBaseDM, function (_aoSuper) {
        return {
            _initMemVar: function (_aoConfig, _aoReplayMail, _aoSendConfig) {
                var _oSelf = this;
                _oSelf._moConfig = _aoConfig;
                ...
                var _oSelf = this, _oConfig = _oSelf._moConfig, _oMailContainer = _oSelf.S(_oConfig.sMailContent),
                    _oMail = _oSelf._moReplyMail,
                    _oValue = [_asSource || textToHtml(htmlEncode(_oSelf.S(_ID._sQSource).value))];
                try {
                ...
                if (!_oConfig.noIncludeArtcle) {
                    ...
                    var _sTmpInclude = _TEMPLATE._REFERPART.replace(extend({}, _oMail, _oConfig.titlePrefix == "1" ? _LANG._EN_US : _LANG._ZH_CN, {
                        'name': _oMail.from.name,
                        'addr': _oMail.from.addr
                    }));
                ...
                var _oSelf = this, _oWin = _oSelf._moWin, _oConfig = _oSelf._moConfig, _oEditor = _oSelf._moEditor,
                    _oSendConfig = _oSelf._moSendConfig, _sDraftContent = _oEditor && _oEditor.getContent(false),
                    _oSource = _oSelf.S(_ID._sQSource);
                if (!_oEditor || _sDraftContent == _oSelf._msDraftContent || !_oSelf._getValidHtmlContent(_sDraftContent) || isShow(_ID._sAfterSendingDiv, _oWin)) {
                    ...
                    var _oReplyMail = _oSelf._moReplyMail, _oCpsData = extend({
                        actiontype: "save",
                        ReAndFw: "reply",
                        contenttype: "html",
                        from_s: "comm_quick",
                        t: "compose.json",
                        ReAndFwMailid: _oSelf.getMailId(),
                        to: _ADDRS.replace({addrs: _oReplyMail.replyTo}),
                        cc: _ADDRS.replace({addrs: _oReplyMail.replyCc}),
                        subject: (_oSelf._moConfig.titlePrefix == "1" ? "Re:" : "\u56DE\u590D\uFF1A") + _oReplyMail.reSubject,
                        savesendbox: 1,
                        sendname: "",
                        sendmailname: _oSelf._moConfig.sendmailname
                    }, _oSelf._combineContent(_sDraftContent));
                    ...
                    waitFor(function () {
                        ...
                        ComposeLib.send(_oCpsData, {
                            onready: function () {
                                _oSelf._disableSendBtn(true);
                                showProcess(1, true, "\u90AE\u4EF6\u6B63\u5728\u4FDD\u5B58\u5230\u8349\u7A3F\u7BB1", "", false);
                            }, oncomplete: function (_abIsOk, _avParam) {
                                var _oQSource = _oSelf._moSource, _oData = evalValue(_avParam),
                                    _sTime = formatDate(new Date(), "%hh%:%mm%", "%");
                                if (_abIsOk) {
                ...
                var _oSelf = this, _oWin = _oSelf._moWin, _oConfig = _oSelf._moConfig, _oEditor = _oSelf._moEditor,
                    _oSendConfig = _oSelf._moSendConfig, _oSource = _oSelf._moSource;
                ...
                if (!_oEditor || !_oSelf._getValidHtmlContent(_oEditor.getContent(false))) {
                    ...
                    var _oReplyMail = _oSelf._moReplyMail, _oCpsData = extend({
                        ...
                        sendname: "",
                        sendmailname: _oSelf._moConfig.sendmailname
                    }, _oSelf._combineContent(_oEditor.getContent(false)));
                    if (_oSelf._msDraftMailId) {
                        _oCpsData.fmailid = _oSelf._msDraftMailId;
                    }
                    waitFor(function () {
                ...
                var _oSelf = this, _oEditor = _oSelf._moEditor,
                    _sValue = _oEditor ? _oEditor.getContent(false) : _oSelf.getSource(), _oWin = _oSelf._moWin,
                    _sUrl = _READMAIL_CPS_URL.replace({
                        sid: getSid(),
                        mailid: _asMailId,
                        s: "reply_all",
                        disptype: "html"
                ...
                _createForm({
                    oWin: _oWin,
                    sTarget: "_self",
                    sAction: _sUrl + (getTop().bnewwin ? "&newwin=true" : "")
                }, {pluscontent: getTop().htmlEncode(_sValue)}).submit();
                preventDefault(_aoEvent);
                ...
                var _oSelf = this, _oWin = _oSelf._moWin, _oSource = _oSelf._moSource,
                    _oSendBtn = _oSelf.S(_ID._sQSendBtn), _oReplyTxtContainer = _oSelf.S(_ID._sQReplyTxtContainer),
                    _oRteContainer = _oSelf.S('rteContainer'), _oReplyBtnContainer = _oSelf.S(_ID._sQReplyBtnContainer);
                ...
                function _expand() {
                    if (_oSource.className.indexOf('graytext') != -1 && attr(_oSource, "checkBcc") != 1) {
                        ...
                        qmAnimation.expand(_oSource, {
                            to: 54, oncomplete: function () {
                                ...
                                if (!_oSelf._moEditor) {
                                    ...
                                    if (!_sInitEditorContent || trim(_sInitEditorContent) == '') {
                                        _sInitEditorContent = QMEditor.getBreakLine(1, {
                                    ...
                                    QMEditor.createEditor({
                                        ...
                                        editorAreaId: 'QMEditorArea' + _sAux,
                                        tbExternId: 'QMEditorToolBarPlusArea' + _sAux,
                                        editorAreaWin: _oWin,
                                        isNoEditScroll: true,
                                        height: "103px",
                                    ...
                                    _oEditorDom.view("show");
                                    var _oEmbed = GelTags("embed", _oSelf.S("QMEditorToolBarPlusArea"))[0],
                                        _oEmbedParent = _oEmbed.parentNode;
                ...
                ;_oSource.view = _view;
                addEvents(setClass(_oSource, 'graytext qm_txt'), {
    ...
    _oClassHome.qmPlayerParser = inherit("qmPlayerParser", _oClassHome._qmBaseDM, function (_aoSuper) {
        return {
            _get: function (_aoPlayerDom, _asAttr) {
                ...
                audioPlay({
                    id: _sUid,
                    container: _oContainer,
                    url: _aoPlayConfig.url,
                    author: _aoPlayConfig.author,
                    title: _aoPlayConfig.title,
                    autoplay: true,
                    global: true
                });
                _oMp3PlayerInfo.innerHTML = _MUSICINFO.replace({
                    images_path: getPath('image'),
                    author: htmlEncode(_aoPlayConfig.author) || '\u672A\u77E5',
                    title: htmlEncode(_aoPlayConfig.title) || '\u672A\u77E5'
                });
                show(_oMp3PlayerInfo, true);
            }, _fPlayBgMusic: function (_asUrl, _aoDisp, _abAutoPlay, _asCmd) {
                var _oSelf = this, _oWin = _oSelf._moWin, _oBgMusic = _oSelf.S(_ID._sBgMusic), _oPlayConfig = {
                    auto: _abAutoPlay,
                    url: _asUrl,
                    sosoGet: _aoDisp ? true : false,
                    author: _aoDisp && _aoDisp.author,
                    title: _aoDisp && _aoDisp.title
                };
                    ...
                    if (_sCmd.indexOf("cmd:") == 0) {
                        ...
                        switch (_sCmd) {
                            case "voice":
                                ...
                                var _sParam = escape(attr(_oPlayer, "param")),
                                    _sMedia = attr(_oPlayer, "media") || "voice", _oSpans = GelTags("span", _oAttach),
                                    _nSpansLen = _oSpans.length, _sPlayUrl = "";
                                    ...
                                    if (_sPlayUrl && (_oSpan.innerText || _oSpan.textContent) == unescape(_sParam)) {
                                        ...
                                        if (_sMedia == "video") {
                                            if (!_oWin.sFlvPlayUrl) {
                                                ...
                                                _oPlayerContainer.innerHTML = generateFlashCode(unikey("flvplayer"), "/zh_CN/htmledition/swf/WebFlvPlayer.swf", {
                                                    width: 400,
                                                    height: 335
                                                }, {wmode: "opaque"});
                                            }
                                            ...
                                            var _oPlayInfo = {
                                                id: _sParam,
                                                container: _oPlayerContainer.firstChild,
                                                url: _sPlayUrl,
                                                title: _oPlayer.getAttribute('alias') ? _oPlayer.getAttribute('alias') + '\u7684\u8BED\u97F3' : '\u60A8\u670B\u53CB\u7684\u8BED\u97F3',
                                                dispInfo: {title: _oPlayer.getAttribute("alias") ? _oPlayer.getAttribute("alias") + "\u7684\u8BED\u97F3" : "\u60A8\u670B\u53CB\u7684\u8BED\u97F3"},
                                                autoplay: false
                                            };
                                            _nIdx--;
                                ...
                                break;
                            case "bgmusic":
                                var _sUrl = attr(_oPlayer, "url"), _sSong = _oSelf._get(_oPlayer, "song"),
                                    _sSinger = _oSelf._get(_oPlayer, "singer");
                                _oSelf._fPlayBgMusic(_sUrl, _sUrl && !_sSong && !_sSinger ? null : {
                                    author: _sSinger,
                                    title: _sSong
                                }, !_aoConfig.bManuPlay, _sCmd);
                                break;
                            case "pcbgmusic":
                                var _sUrl = attr(_oPlayer, "url"), _sSong = _oSelf._get(_oPlayer, "song"),
                                    _sSinger = _oSelf._get(_oPlayer, "singer"),
                                    _oDiv = _oWin.document.createElement('div');
                ...
                E(SN(_NAME._sMoreOprContainer, _aoConfig.oWin), function (_aoDom) {
                    new QMSelect({
                        ...
                        oMenu: {
                            ...
                            nMaxWidth: 180,
                            nMaxItemView: 10,
                            oItems: QMMenu.makeMenuItem(_oText, _oValue)
                        },
                        onafteropenmenu: function (_oMenu, _aoSelect) {
                            for (var _i = _oValue.length - 1; _i >= 0; _i--) {
    ...
    _oClassHome.qmRemark = inherit("qmRemark", _oClassHome._qmBaseDM, function (_aoSuper) {
        return {
            _initMemVar: function (_afOnChange) {
                var _oSelf = this;
                ...
                var _oSelf = this, _oRemarkText = _oSelf.S(_ID._sRemarkText),
                    _nScrollHeight = _oRemarkText.scrollHeight;
                if (gbIsIE) {
                ...
                var _oSelf = this, _oRmd = _oSelf.S(_ID._sRmd), _oRemarkContainer = _oSelf.S(_ID._sRemarkContainer),
                    _oRemarkText = _oSelf.S(_ID._sRemarkText), _oRemarkWrite = _oSelf.S(_ID._sRemarkWrite);
                ...
                var _oSelf = this, _oRmd = _oSelf.S(_ID._sRmd), _oRemarkContent = _oSelf.S(_ID._sRemarkContent),
                    _oRemarkContainer = _oSelf.S(_ID._sRemarkContainer), _oRemarkText = _oSelf.S(_ID._sRemarkText),
                    _oRemarkWrite = _oSelf.S(_ID._sRemarkWrite), _oRemarkRead = _oSelf.S(_ID._sRemarkRead);
                ...
                confirmBox({
                    ...
                    onreturn: function (_abIsOk) {
                        if (_abIsOk) {
                            ...
                            _oAjax.send(T("mailaction=remarks&sid=$sid$&type=del&mailid=$mailid$").replace({
                                sid: getSid(),
                                mailid: _sMailId
                            }));
                        }
                ...
                var _oSelf = this, _oWin = _oSelf._moWin, _oRemarkContent = _oSelf.S(_ID._sRemarkContent),
                    _oRemarkText = _oSelf.S(_ID._sRemarkText), _oRemarkWrite = _oSelf.S(_ID._sRemarkWrite),
                    _oRemarkRead = _oSelf.S(_ID._sRemarkRead),
                    _oAjax = new QMAjax("/cgi-bin/mail_mgr?mailaction=remarks&type=mdy"), _sText = _oRemarkText.value;
                ...
                _oSelf._changeButtonMode(true);
                _oAjax.send(T("mailaction=remarks&sid=$sid$&resp_charset=UTF8&type=mdy&mailid=$mailid$&content=$content$").replace({
                    sid: getSid(),
                    mailid: _oSelf.getMailId(),
                    content: encodeURI(_sText)
                }));
                return false;
            }, cancel: function () {
                var _oSelf = this, _oRemarkContent = _oSelf.S(_ID._sRemarkContent),
                    _oRemarkContainer = _oSelf.S(_ID._sRemarkContainer), _oRemarkText = _oSelf.S(_ID._sRemarkText),
                    _oRemarkWrite = _oSelf.S(_ID._sRemarkWrite), _oRemarkRead = _oSelf.S(_ID._sRemarkRead);
                if (_oRemarkContent.innerHTML == "") {
                ...
                var _oSelf = this, _oRemarkContent = _oSelf.S(_ID._sRemarkContent),
                    _oRemarkText = _oSelf.S(_ID._sRemarkText);
                if (!_oRemarkText) {
                    return '';
                }
        ...
        return {
            _initMemVar: function (_aoConfig, _aoMailInfo) {
                ...
                _createForm({
                    ...
                    reporttype: "",
                    location: "readmail",
                    srcfolderid: this._moConfig.folderid,
                    mailaction: "mail_spam"
                }, _aoExtend));
            }, reject: function (_aoTarget, _asMailId) {
                var _oSelf = this, _sTypeReject = attr(_aoTarget, "type_reject"),
                    _sSubjectId = attr(_aoTarget, "subjectid");
                ...
                if (_sTypeReject == "sys_reject") {
                    ...
                    doReject(true, attr(_aoTarget, "groupmail"), _oSelf._moWin, attr(_aoTarget, "mimefrom") || _oFrom.addr, T(['<div>\u62D2\u6536\u540E\u60A8\u5C06\u4E0D\u518D\u6536\u5230\u6765\u81EA\u4EE5\u4E0B\u5730\u5740\u7684\u90AE\u4EF6\uFF0C\u786E\u8BA4\u62D2\u6536\u5417\uFF1F</div>', '<div for="reject_confirm_$_idx_$" class="txtflow" style="width:320px;">', '<span class="green bold">$name$</span>', '<span class="graytext">&lt;$addr$&gt;</span>', '</div>']).replace({
                        name: htmlEncode(_oFrom.name),
                        addr: _oFrom.addr
                    }));
                }
                ...
                reportNoSpamJson({}, {
                    oWin: _oSelf._moWin,
                    sFid: _oSelf._moConfig.folderid,
                    oACB: null,
                    bPop: false,
                    bML: false,
                    oMail: [{sMid: _asMailId, bUnr: false}]
                });
                ...
                reportSpamJson({bBlackList: _bNeedBlack, oAddrList: _lAddrs}, {
                    oWin: _oSelf._moWin,
                    sFid: _oSelf._moConfig.folderid,
                    oACB: null,
                    bPop: false,
                    bML: false,
                    oncomplete: function (_aoCBInfog) {
                ...
                _createForm({
                    oWin: _oWin,
                    sMethod: "POST",
                    sAction: _BW_LIST.replace({sid: getSid(), fromaddr: _sFromAddr})
                }).submit();
                ...
                runUrlWithSid(_SPAM_REPORT.replace({
                    rtype: _sType,
                    rmsg: _asMailId
                }) + "&r_subtype=spamvote&fname=" + _oSelf.getFromInfo_().addr);
                if (_sType == "1") {
                ...
                var _oSelf = this, _sBook = _oSelf.attr(_aoTarget, "book"),
                    _nIsTuan = _oSelf.attr(_aoTarget, "tuan") || 0;
                loadingBox({
                    model: "\u53CD\u5783\u573E", js: ["$js_path$qmantispam264eed.js"], oncheck: function () {
                        return !!getTop().QMAntiSpam;
                    }, onload: function () {
                        var _oSpam = new QMAntiSpam.qmExbookEmlMgr({
                            sMailId: _asMailId, from: _oSelf.getFromInfo_(), fOnReload: function () {
    ...
    _oClassHome.qmMarkAdMail = inherit("qmMarkAdMail", _oClassHome._qmBaseDM, function (_aoSuper) {
        return {
            _initMemVar: function (_aoConfig, _aoMailInfo) {
                ...
                reportAdJson({bNotAd: !_bMarkAd, bShowConfirm: true}, {
                    oWin: _oSelf._moWin,
                    sFid: _oConfig.folderid,
                    oACB: null,
                    bPop: false,
    ...
    _oClassHome.qmSenderInfo = inherit("qmSenderInfo", _oClassHome._qmBaseDM, function (_aoSuper) {
        return {
            _initMemVar: function (_aoConfig, _aoMailInfo) {
                ...
                var _oSelf = this;
                createIframe(_oSelf._moWin, _S_RD_LIST.replace({
                    sid: getSid(),
                    mail: _oSelf._moMail
                }), {id: "iframeRss"});
            }, _getMailList: function () {
                var _oSelf = this;
                QMAjax.send(_S_ML_LIST.replace({sid: getSid(), mailid: _oSelf.getMailId(), mail: _oSelf._moMail}), {
                    method: "GET", onload: function (_abIsOk, _asParam) {
                        if (_abIsOk) {
                            var _oJson = evalValue(_asParam);
                ...
                var _oSelf = this, _oConfig = _oSelf._moConfig, _oPanelInfo = _oSelf.S(_ID._sSenderInfo),
                    _bShow = !isShow(_oPanelInfo);
    ...
    _oClassHome.qmConvMail = inherit("qmConvMail", _oClassHome.qmReadMail, function (_aoSuper) {
        return {
            _initMemVar: function () {
                ...
                switch (_oContext.sModuleName) {
                    case "qmSubMail":
                        var _oSubMail = _oMailInfo.oSubMails[_oContext.sAux] || [{}, {}],
                            _sContext = _oContext.sContext || "";
                    ...
                    case "qmQReply":
                        ...
                        _oModule = _oSelf._moQReply = new _oClassHome.qmQReply(_oSubMail[0], _oSubMail[1], {
                            fCheckBcc: function () {
                                callBack.call(_oSelf, _oSelf.checkBcc, arguments);
                            ...
                            }, s: "conv_send", sourceContent: _sInitEditorContent
                        }, _oContext);
                        break;
                    case "qmAntiSpam":
                ...
                _oSelf._startSubMod({
                    sContext: _oMailLists[0].getAttribute("context"),
                    sModuleName: "qmSubMail",
                    sAux: _oMailLists[0].getAttribute("aux")
                });
                ...
                var _oSelf = this, _oContentDiv = _oSelf.S('submail_inner_body'),
                    _oFoldDivs = finds('div[ck="dispSubMail"]', _oContentDiv),
                    _oFirstDiv = _oSelf.parentToContextDom(_oFoldDivs[0]);
                if (_oFirstDiv) {
                    _asMailId = _oFirstDiv.getAttribute('context');
                }
                _aoSuper.optMail.call(_oSelf, _aoTarget, _asMailId, _aoEvent);
            }, _pageReady: function () {
                var _oSelf = this, _oWin = _oSelf._moWin, _oConfig = _oSelf._moConfig, _oMailInfo = _oSelf._moMailInfo,
                    _oSubMails = _oMailInfo.oSubMails;
                _oSelf._fakeReadMail();
                _oSelf._readMailFinish();
                _oSelf._rInsPageEnd();
                _oSelf._rInsAsyn();
                ...
                var _oSelf = this;
                doRmMail(extend(_oSelf.getCBInfo(), {
                    oncomplete: function (_aoCBInfo, _aoResult) {
                        _oSelf.dealCustomUI('MailMethod', [_aoCBInfo, 'delMail', _aoResult]);
                        var _sUrl = _aoResult.url || "";
                        if (_sUrl.indexOf("/cgi-bin/readmail?") != -1) {
                            ...
                            var _sType = (_sTemplate == "readmail_ad" || _sTemplate == "readmail_ad_conversation") ? "collo" : "";
                            _oSelf.prevandnext({
                                mailid: getUrlParams(_sUrl || _oSelf._moWin.location)["mailid"],
                                type: _sType
                            });
                            return true;
                        }
                ...
                var _oSelf = this, _nCount = finds("input[ck='selectSubMail']", _oSelf.S("submail_inner_body")).length,
                    _bIsPerDel = _oSelf.attr(_aoTarget, "opt") == "1";
                ...
                new QMDialog({
                    sTitle: "\u5220\u9664\u786E\u8BA4",
                    sBodyHtml: TE(['<div>', '<div class="cnfx_content">', '<span class="dialog_icon icon_info_b"></span>', '<div class="dialog_f_c">', '$@$if($isperdel$)$@$', '$@$if($count$>1)$@$', '<div class="dialog_f_t">\u60A8\u786E\u5B9A\u8981\u5F7B\u5E95\u5220\u9664\u8FD9<span class="bold" style="margin:4px;">$count$</span>\u5C01\u90AE\u4EF6\u5417\uFF1F</div>', '<div class="dialog_f_d">\u5F7B\u5E95\u5220\u9664\u540E\u5C06\u65E0\u6CD5\u6062\u590D\uFF0C\u60A8\u4E5F\u53EF\u4EE5\u901A\u8FC7\u6574\u7406\u90AE\u4EF6\u8FDB\u884C\u90E8\u5206\u5220\u9664\u3002</div>', '$@$else$@$', '\u5F7B\u5E95\u5220\u9664\u540E\u90AE\u4EF6\u5C06\u65E0\u6CD5\u6062\u590D\uFF0C\u60A8\u786E\u5B9A\u8981\u5220\u9664\u5417\uFF1F', '$@$endif$@$', '$@$else$@$', '$@$if($count$>1)$@$', '<div class="dialog_f_t">\u60A8\u786E\u5B9A\u8981\u5220\u9664\u8FD9<span class="bold" style="margin:4px;">$count$</span>\u5C01\u90AE\u4EF6\u5417\uFF1F</div>', '<div class="dialog_f_d">\u5220\u9664\u540E\u5C06\u79FB\u5230\u201C\u5DF2\u5220\u9664\u201D\u4E2D\uFF0C\u60A8\u4E5F\u53EF\u4EE5\u901A\u8FC7\u6574\u7406\u90AE\u4EF6\u8FDB\u884C\u90E8\u5206\u5220\u9664\u3002</div>', '$@$else$@$', '\u5220\u9664\u540E\u90AE\u4EF6\u5C06\u79FB\u5230\u201C\u5DF2\u5220\u9664\u201D\u4E2D\uFF0C\u60A8\u786E\u5B9A\u8981\u5220\u9664\u5417\uFF1F', '$@$endif$@$', '$@$endif$@$', '<div></div>', '</div>', '</div>']).replace({
                        isperdel: _bIsPerDel,
                        count: _nCount
                    }),
                    sFootHtml: TE(['<div class=" txt_right cnfx_btn">', '$@$if($count$>1)$@$', '<a class="btn_gray btn_left" id="selectmail" href="javascript:;">\u6574\u7406\u90AE\u4EF6<span class="btn_dots">...</span></a>', '$@$endif$@$', '<a class="btn_gray confirm wd2" id="confirm" href="javascript:;">\u786E\u5B9A</a>', '<a class="btn_gray cancelwd2" id="cancel" href="javascript:;">\u53D6\u6D88</a>', '</div>']).replace({count: _nCount}),
                    onshow: function () {
                        var _oDlg = this;
                ...
                rmMail(_bIsPerDel, extend(_oSelf.getCBInfo(), {
                    oncomplete: function (_aoCBInfo, _aoResult) {
                        var _sUrl = _aoResult.url || "";
                        var _sTemplate = getUrlParams(_sUrl || _oSelf._moWin.location)["t"];
                        var _sType = (_sTemplate == "readmail_ad" || _sTemplate == "readmail_ad_conversation") ? "collo" : "";
                ...
                for (var i = 0; i < _oMailInfo.nLen; ++i) {
                    ...
                    if (_bIsSelf != 1) {
                        _oMail.push({
                            sSName: _oTempFrom.name,
                            sSEmail: _oTempFrom.addr,
                            sMid: _oMailInfo.oSubMails[i][2].sContext
                        });
                    }
                }
                reportAddrConfirm({oMail: _oMail}, {
                    sTitle: '\u62D2\u6536\u786E\u8BA4',
                    sConfirmBtnTxt: '\u62D2\u6536',
                    sTextTitle: '\u62D2\u6536\u540E\u60A8\u5C06\u4E0D\u518D\u6536\u5230\u6765\u81EA\u4EE5\u4E0B\u5730\u5740\u7684\u90AE\u4EF6\uFF0C\u786E\u8BA4\u62D2\u6536\u5417\uFF1F',
                    onreturn: function (_abOK, _oAddrs, _oMailIds, _oHashMailIdMap) {
                        if (_abOK) {
                            getTop().QMMLCache.upVer();
                            _oSelf._moAntiSpam || _oSelf._startSubMod({sModuleName: "qmAntiSpam"});
                            _oSelf._moAntiSpam._createMailFrm(_oSelf.getMailId(), {
                                "addr": _oAddrs.join(";"),
                                "submailid": _oMailIds.join(";"),
                                "nogopage": _oSelf.dealCustomUI('getModelName') == 'floatRead4mailList' ? 'true' : ''
                            });
                ...
                var _oSelf = this, _oDomFoldIcon = _aoTarget, _oContentDiv = _oSelf.S("submail_inner_body"),
                    _oFoldDivs = finds("div[ck='dispSubMail']", _oContentDiv), _bAllFolded = true, _oMailDiv, _oSubMail;
                ...
                ;E(finds("input[ck='selectSubMail']", _oSubMailsDiv), function (_aoDom) {
                    if (_aoDom.checked) {
                        ...
                        _oMailIds.push({
                            sMid: attr(_oContentDiv, "context"),
                            bSys: _oConfig.bSys,
                            bUnr: attr(_oContentDiv, "newmail") == 'true',
                            bSubUnr: parseInt(attr(_oContentDiv, "newcnt") || '0', 10) > 0,
                            bTms: false,
                            sSName: _oFrom.name,
                            sSEmail: _oFrom.addr,
                            sDOMid: _oContentDiv.id
                        });
                    }
                });
                return {
                ...
                var _oSelf = this, _bPerl = attr(_aoTarget, "opt") == "1", _bAll = _oSelf.S("selectAllSubMail").checked,
                    _oMailIds = [], _oIds = [], _nTotal = 0, _oSubMailsDiv = _oSelf.S("submail_inner_body");
                E(finds("input[ck='selectSubMail']", _oSubMailsDiv), function (_aoDom) {
                    if (_aoDom.checked) {
                        var _oContentDiv = _oSelf.parentToContextDom(_aoDom);
                ...
                if (_bPerl) {
                    confirmBox({
                        msg: T(['<div class="dialog_f_t">\u60A8\u786E\u5B9A\u8981\u5F7B\u5E95\u5220\u9664\u6240\u9009\u7684<span class="bold" style="margin:4px;">$count$</span>\u5C01\u90AE\u4EF6\u5417\uFF1F</div>', '<div class="dialog_f_d">\u5F7B\u5E95\u5220\u9664\u540E\u90AE\u4EF6\u5C06\u65E0\u6CD5\u6062\u590D\u3002</div>']).replace({count: _oMailIds.length}),
                        title: "\u5220\u9664\u786E\u8BA4",
                        cancelBtnTxt: "\u53D6\u6D88",
                        confirmBtnTxt: "\u786E\u5B9A",
                        onreturn: function (_abIsOk) {
                ...
                QMAjax.send(T('/cgi-bin/readmail?sid=$sid$&s=$submail$&isAdMail=$isAdMail$&t=check_submail.json&mode=pre&action=checksubmail&mailid=$mailid$&submailid=$submailid$').replace({
                    sid: getSid(),
                    mailid: _sConvMailId,
                    submailid: _asMailId,
                    isAdMail: _oSelf._msModuleName == 'qmAdConvMail' ? 1 : 0,
                    submail: _oSelf._msModuleName == 'qmAdConvMail' ? 'adsubmail' : 'submail'
                }), {
                    method: 'GET', onload: function (_abIsOk, _asParam) {
                        var _oJson = evalValue(_asParam);
                        if (_abIsOk) {
                ...
                _oSelf._startSubMod({
                    sContext: _oCurrentContent.sContext,
                    sModuleName: _oCurrentContent.sModuleName,
                    sAux: _oCurrentContent.sAux
                });
            }, adjustTipsPosition: function (_abInit, _aoReferPosition) {
                var _oSelf = this, _oTop = getTop(), _nFixedTop = 12,
                    _oTipsDiv = finds('a[ck="checkNewSubMail"]', _oSelf._moWin)[0];
                ...
                var _oSelf = this, _oTop = getTop(), _oTipsDiv = finds('a[ck="checkNewSubMail"]', _oSelf._moWin),
                    _oAllCountDiv = finds('a[ck="foldSubMail"]', _oSelf._moWin);
                if (_abShow) {
    ...
    _oClassHome.qmSubMail = inherit("qmSubMail", _oClassHome.qmReadMail, function (_aoSuper) {
        return {
            _init: function () {
                ...
                var _oTop = getTop(), _oSelf = this, _oWin = _oSelf._moWin, _oConfig = _oSelf._moConfig,
                    _oMailInfo = _oSelf._moMailInfo, _oContentDiv = _oSelf.S(_ID._sContentDiv);
                if (!_oMailInfo.bAsyn) {
                ...
                var _oSelf = this;
                QMAjax.send(T("/cgi-bin/readmail?sid=$sid$&t=readsubmail&s=$s$&mailid=$mailid$&submailid=$submailid$&frid=$frid$&classalias=qmbox$index$&index=$index$&folderidAlias=$folderidAlias$" + (_asQuery ? '&' + _asQuery : '')).replace({
                    sid: getSid(),
                    mailid: _oSelf.getCMailId_(),
                    submailid: _oSelf.getMailId(),
                    frid: _oSelf._moMailInfo.frid,
                    s: _asType,
                    index: _oSelf._moContext.sAux,
                    folderidAlias: getTop().S("ipt_folderid", getMainWin()) && getTop().S("ipt_folderid", getMainWin()).value
                }), {
                    method: "GET",
                    headers: {"If-Modified-Since": "0", "Cache-Control": "no-cache, max-age=0"},
                    onload: function (_abIsOk, _asParam) {
                ...
                var _oSelf = this, _sInner = _aoFormRef.innerHTML, _nIdx = 0,
                    _oForms = GelTags("form", _oSelf.S(_ID._sContentDiv));
                ...
                getTop().QMMLCache.upVer();
                goNewWin(T([_READMAIL_, '&t=readmail&folderid=$folderid$&folderidAlias=$folderidAlias$']).replace({
                    folderid: this._moConfig.folderid,
                    mailid: _asMailId,
                    folderidAlias: getTop().S("ipt_folderid", getMainWin()) && getTop().S("ipt_folderid", getMainWin()).value
                }), false);
            }, delMail: function (_aoTarget) {
                ...
                rmMail(_oSelf.attr(_aoTarget, "opt") || 0, extend(_oSelf.getCBInfo(), {
                    oncomplete: function (_aoCBInfo, _aoResult) {
                ...
                var _oPos = calcPos(_aoTarget), _oSelf = this, _oWin = _oSelf._moWin,
                    _oDocElem = _oWin.document && _oWin.document.documentElement, _oItems, _nY = _oPos[2],
                    _nMenuHeight = 185, _nWindowHeight = _oDocElem ? (_oDocElem.clientHeight + _oDocElem.scrollTop) : 0;
                if (_oSelf._moConfig.bEncrypt) {
                    _oItems = [{sId: "reportSpam", sItemValue: "\u4E3E\u62A5"}, {
                        sId: "PerDel",
                        sItemValue: "\u5F7B\u5E95\u5220\u9664"
                    }];
                }
                ...
                var _oSelf = this, _oMailInfo = _oSelf._moMailInfo, _sDisp = _oSelf.attr(_aoTarget, "disp"),
                    _oDetailBtn = _oSelf.S(_ID._sDetailBtn), _oDetail = _oSelf.S(_ID._sDetail),
                    _oSum = _oSelf.S(_ID._sSum), _oImg = GelTags("img", _oDetailBtn)[0],
                    _oA = GelTags("a", _oDetailBtn)[0], _oReferInfo = _oSelf.S(_ID._sReferInfo);
                if (_sDisp == "1") {
                    if (isShow(_oDetail)) {
                        return;
                    }
                ...
                debug("_fandx");
                var _oSelf = this, _oTop = getTop(), _oFold = _oSelf.S(_ID._sFold), _oExpand = _oSelf.S(_ID._sExpand),
                    _oSubMailDiv = _oSelf.S(_ID._sSubMail);
                if (_abFold == 'auto') {
                ...
                if (_abFold) {
                    ...
                    show(_oFold, false);
                    qmAnimation.fold(_oExpand, {
                        speed: "fast", to: _nTo || 48, oncomplete: function () {
                    ...
                    var _oDomAttCon = _oTop.finds("div[ui-type='attCon']", _oSubMailDiv)[0],
                        _oDomAtt = _oDomAttCon && _oTop.finds("a[ui-type='netdiskBind']", _oDomAttCon)[0];
                    _oTop.QMNetDisk && _oDomAttCon && _oDomAtt && setTimeout(function () {
                        _oTop.QMNetDisk.route("tips", "readmail", _oDomAttCon, {
                            offsetTop: 80,
    ...
    _oClassHome.qmAdConvMail = inherit("qmAdConvMail", _oClassHome.qmConvMail, function (_aoSuper) {
        return {
            _pageReady: function () {
                ...
                starMail(_bStar, extend(_oSelf.getCBInfo(), {
                    oncomplete: function (_aoCBInfog, _abIsSet) {
    ...
    _oClassHome.qmAdSubMail = inherit("qmAdSubMail", _oClassHome.qmSubMail, function (_aoSuper) {
        return {
            _asyncGetSubMail: function (_asQuery) {
                ...
                var _oSelf = this, _oConfig = _oSelf._moConfig,
                    _sMenuId = ["menu", _oConfig.cmailid, _oConfig.sIndex].join("_"), _sMailId = _oSelf.getMailId();
                new QMMenu({
                    sId: _sMenuId,
                    oEmbedWin: _oSelf._moWin,
                    oItems: _oSelf._generateMenuItem(),
                    onitemclick: function (_asId, _aoItemConfig) {
                        var _sFolderId = 0;
                        ...
                        switch (_asId) {
                            ...
                            case "reply_all":
                            case "forward":
                                _sCpsMailUrl = _READMAIL_CPS_URL.replace({
                                    sid: getSid(),
                                    s: _asId,
                                    mailid: _sMailId,
                                    disptype: _oConfig.disptype == "text" ? "" : "html"
                            ...
                            case "markAsTag":
                                ...
                                break;
                            case "newTag":
                                QMTag.newMailTag(extend(_oSelf.getCBInfo(), {
                                    oncomplete: function (_aoParams, _aoResult) {
                                    }
                                }));
                                break;
                            case "newFolder":
                            case "moveToFolder":
                                moveMailJs(_asId === "moveToFolder" ? _aoItemConfig.sFolderId.split("_")[1] : "new", _asId === "moveToFolder" ? _aoItemConfig.sItemValue : "", _oSelf._moMailInfo.folderid, extend(_oSelf.getCBInfo(), {
                                    oncomplete: function () {
                                        _oSelf._fandx(true, function () {
                                            removeSelf(_oSelf.S(_ID._sSubMail));
                    ...
                    nWidth: (getLocale() == "zh_CN") ? 100 : 160,
                    nMaxWidth: 180,
                    nMaxItemView: 15,
                    bAnimation: true,
                    onload: function () {
                        if (this.option("sId") === _sMenuId) {
                            var _oMenuObj = this, _nHeight = parseInt(_oMenuObj.option("nHeight"), 10),
                                _nWidth = parseInt(_oMenuObj.option("nWidth"), 10), _oBtnPos = calcPos(_aoTarget),
                                _oMenuPos = calcAdjPos(_oBtnPos.slice(0, 4), _nWidth, _nHeight, _oSelf._moWin, 3);
                ...
                var _oSelf = this;
                goNewWin(T([_READMAIL_, '&t=readmail&folderid=$folderid$$ext$']).replace({
                    folderid: _oSelf._moConfig.folderid,
                    mailid: _asMailId,
                    ext: _oSelf._moMailInfo.bShowContImg || getTop().getUrlParams(_oSelf._moWin.location).dispimg == 1 ? '&disptype=html&dispimg=1&clickshowimage=1' : ''
                }), false);
            }, _generateMenuItem: function () {
                var _oSelf = this, _oItems = [], _oHr = {
                    nHeight: 10,
                    sItemValue: '<div style="background:#CCC; height:1px; margin-top:7px; overflow:hidden;"></div>'
                }, _bSys = _oSelf.getCBInfo().oMail[0] && _oSelf.getCBInfo().oMail[0].bSys || false;
                _bSys || _oItems.push({sId: "reply", sItemValue: "\u56DE\u590D"}, {
                    sId: "reply_all",
                    sItemValue: "\u56DE\u590D\u5168\u90E8"
                });
                _oItems.push({sId: "forward", sItemValue: "\u8F6C\u53D1"}, _oHr, {
                    sId: "deleteMail",
                    sItemValue: "\u5220\u9664"
                }, {sId: "predeleteMail", sItemValue: "\u5F7B\u5E95\u5220\u9664"});
                ...
                var _oSelf = this, _oMailInfo = _oSelf._moMailInfo, _oDetail = _oSelf.S(_ID._sDetail),
                    _oSum = _oSelf.S(_ID._sSum), _oReferInfo = _oSelf.S(_ID._sReferInfo);
                if (isShow(_oDetail)) {
                    return;
                }
                show(_oSum, false);
    ...
    _oClassHome.qmGroupQReply = inherit("qmGroupQReply", _oClassHome._qmBaseDM, function (_aoSuper) {
        return {
            _initMemVar: function (_aoConfig, _aoReplayMail, _aoSendConfig) {
                ...
                var _oSelf = this, _oSource = _oSelf._moSource, _oWin = _oSelf._moWin, _oTop = getTop(),
                    _oSendBtn = _oSelf.S(_ID._sQSendBtn);
                ...
                addEvent(_oSource, "focus", function (_aoEvent) {
                    if (!_oSelf._moEditor) {
                        QMEditor.createEditor({
                            ...
                            size: goUserInfo.get("DEF_FONT_SIZE"),
                            color: goUserInfo.get("DEF_FONT_COLOR")
                        }), false, _oTop.S("QMEditorArea", _oWin).getAttribute("tIndex"));
                        var _oLoadScriptFiles = [];
                        if (!_oTop.ComposeLib) {
                            _oLoadScriptFiles.push("$js_path$libcompose3847ad.js");
                        }
                ...
                if (!_oEditor || _sDraftContent == _oSelf._msDraftContent || isShow(_ID._sAfterSendingDiv, _oWin)) {
                    ...
                    var _oReplyMail = _oSelf._moReplyMail, _oCpsData = {
                        actiontype: "save",
                        t: "compose_send.json",
                        s: "group",
                        qqgroupid: _oReplyMail.from.addr,
                        groupname: _oReplyMail.from.name,
                        subject: _oReplyMail.subject,
                        content__html: _sDraftContent,
                        fmailid: _oSelf.getMailId()
                    };
                    waitFor(function () {
                        return !!(getTop().ComposeLib);
                    }, function (_abWaitOK) {
                        ...
                        ComposeLib.send(_oCpsData, {
                            sType: "group", onready: function () {
                                _oSelf._disableSendBtn(true);
                                showProcess(1, true, "\u90AE\u4EF6\u6B63\u5728\u4FDD\u5B58\u5230\u8349\u7A3F\u7BB1", "", false);
                            }, oncomplete: function (_abIsOk, _avParam) {
                                var _oQSource = S(_ID._sQSource, _oWin), _oData = evalValue(_avParam),
                                    _sTime = formatDate(new Date(), "%hh%:%mm%", "%");
                                if (_abIsOk) {
                                    _oSelf._msDraftMailId = _oData.mailid;
                                    _oSelf._msDraftContent = _sDraftContent;
                ...
                var _oSelf = this, _oSource = _oSelf._moSource, _oEditor = _oSelf._moEditor,
                    _oSendBtn = _oSelf.S(_ID._sQSendBtn), _bFold = isShow(_oSource);
                if (_bFold == _abFold) {
                    return;
                }
                ...
                if (_abFold) {
                    ...
                    _oSendBtn.focus();
                    _oSendBtn.blur();
                    _oEditor.setContent(QMEditor.getBreakLine(1, {
                        family: goUserInfo.get("DEF_FONT_FAMILY"),
                        size: goUserInfo.get("DEF_FONT_SIZE"),
                        color: goUserInfo.get("DEF_FONT_COLOR")
                    }));
                ...
                var _oSelf = this, _oWin = _oSelf._moWin, _oEditor = _oSelf._moEditor, _oConfig = _oSelf._moConfig,
                    _oSendConfig = _oSelf._moSendConfig, _oSource = _oSelf.S(_ID._sQSource);
                if (_aoTarget.disabled) {
                    return;
                }
                if (!_oEditor || !_oSelf._getValidHtmlContent(_oEditor.getContent(false))) {
                    ...
                    var _oReplyMail = _oSelf._moReplyMail, _oCpsData = {
                        qqgroupid: _oConfig.gid + "@groupmail.qq.com",
                        subject: _oReplyMail.subject || "\u65E0\u4E3B\u9898",
                        content__html: _oEditor.getContent(),
                        t: "compose.json",
                        s: "group_send",
                        fmailid: _asMailId
                    };
                    waitFor(function () {
                        ...
                        var _oBackGroundSend,
                            _bShouldBackSend = ("1" == getTop().gbBackGroundSend && (_oBackGroundSend = getTop().BackGroundSend)),
                            _oConfig = {
                                sType: "group", onready: function () {
                                    if (_bShouldBackSend) {
    ...
    _oClassHome.qmGroupMail = inherit("qmGroupMail", _oClassHome.qmReadMail, function (_aoSuper) {
        return {
            _pageReady: function () {
                var _oTop = getTop(), _oSelf = this, _oWin = _oSelf._moWin, _oMailInfo = _oSelf._moMailInfo,
                    _oConfig = _oSelf._moConfig,
                    _oContentDivs = CN(_ID._sContentClassName, getTop().getMainWin().document);
                _oSelf._fakeReadMail();
                ...
                var _oSelf = this, _oModule, _oConfig = _oSelf._moConfig, _oMailInfo = _oSelf._moMailInfo,
                    _oContext = extend({}, _oSelf._moContext, _aoSubMod, {oMailInstance: _oSelf});
                switch (_oContext.sModuleName) {
                    case "qmGroupQReply":
                        _oModule = _oSelf._moGroupQReply = new _oClassHome.qmGroupQReply(_oConfig, _oMailInfo, {
                            fQReplyComplete: function (_avParam) {
                                _oSelf._qReplyComplete(_avParam);
                            }
                        }, _oContext);
                ...
                return _MAIL_LIST1.replace({
                    ...
                    folderid: _oConfig.folderid,
                    s: _oConfig.subtmpl,
                    more: "&t=mail_list_group&groupid=" + (_oConfig.groupid || "")
                });
                ...
                var oSettingGroup = new QMMenu({
                    oEmbedWin: _oSelf._moWin,
                    sId: "menu_" + _ID._sSettingGroup,
                    nWidth: getLocale() == "zh_CN" ? 290 : 454,
                    oItems: [{
                        nHeight: "auto",
                        sItemValue: _SETTING_GROUP_TMP.replace({
                            bReject: _oApp._moConfig.bReject,
                            bNewreply: _oApp._moConfig.bNewreply,
                            bNotify: _oApp._moConfig.bNotify,
                            sid: getSid()
                        })
                    }],
                    nX: _oPos[3],
                    ...
                    onload: function () {
                        ...
                        addEvent(_oSelf.S("save"), "click", function () {
                            ...
                            QMAjax.send(T(['/cgi-bin/grouplist?t=mail_mgr2&mailaction=groupmgr', '&oper1=$oper1$&oper2=$oper2$&sid=$sid$&gid=$gid$@groupmail.qq.com']).replace({
                                oper1: _oResult["qqnotify"] || "",
                                oper2: _oResult["unread"] || "",
                                sid: getSid(),
                                gid: _oApp._moConfig.gid
                            }), {
                                method: "GET",
                                headers: {"If-Modified-Since": "0", "Cache-Control": "no-cache, max-age=0"},
                                onload: function (_abIsOk, _asParam) {
                ...
                QMAjax.send(_REJECT_GROUP.replace({sid: getSid(), gid: _oSelf._moConfig.gid, yn: _sOpt}), {
                    method: "GET",
                    headers: {"If-Modified-Since": "0", "Cache-Control": "no-cache, max-age=0"},
                    onload: function (_abIsOk, _asParam) {
                        var _oJson = evalValue(_asParam);
                ...
                var _oTop = getTop(),
                    _sUrl = T("/cgi-bin/groupmail_close?groupid=$gid$&mailid=$mailid$&action=$action$&sid=$sid$").replace({
                        gid: _oTop.attr(_aoTarget, 'gid'),
                        mailid: _asMailId,
                        action: _oTop.attr(_aoTarget, 'action'),
                        sid: _oTop.getSid()
                    });
                _oTop.QMAjax.send(_sUrl, {
                    onload: function (_abIsOk, _asParam) {
                        if (_abIsOk) {
                ...
                if (_sVid = _aoTarget.getAttribute("vid")) {
                    ...
                    var _oVoteDisplay = _oSelf.S("voter_play_box"), _oTrs = GelTags("tr", _oVoteDisplay),
                        _bShowOpt = (_abShow == undefined || (typeof _abShow == "string")) ? !_aoTarget.getAttribute("opt") : _abShow;
    ...
    _oClassHome.qmAdSubMailML = inherit("qmAdSubMailML", _oClassHome.qmAdSubMail, function (_aoSuper) {
        ...
        return {
            ...
            _asyncGetMail: function (_asType, _afCallBack, _asQuery) {
                var _oSelf = this;
                QMAjax.send(_oSelf.getAsyncMailUrl() + (_asQuery ? _asQuery : ''), {
                    method: "GET", onload: function (_abIsOk, _asParam) {
                        var _sText = trim(_asParam);
                        if (!_abIsOk || _sText.indexOf("<!--cgi exception-->") == 0) {
                            _afCallBack();
            ...
            getAsyncMailUrl: function (_abNoCGI) {
                var _oSelf = this;
                var _sMailId = _oSelf.getCMailId_();
                return T("$cgi$sid=$sid$&t=readsubmail&mailid=$mailid$&frid=$frid$&index=$index$&fromModule=qmAdSubMailML&classalias=qmbox$index$&s=$s$&maxage=$maxage$&base=$base$&ver=$ver$").replace({
                    cgi: _abNoCGI ? '' : '/cgi-bin/readmail?',
                    sid: getSid(),
                    mailid: _sMailId,
                    frid: _oSelf._moMailInfo.frid,
                    index: _oSelf._moContext.sAux,
                    s: _sMailId.charAt(0) == 'C' ? 'quickreadmail' : '',
                    maxage: rdVer.maxage(_sMailId),
                    base: rdVer("BaseVer", 0),
                    ver: rdVer(_sMailId, 0, 234)
                });
            },
            ...
            _nIntervalFandx2scroll: 0,
            _nFandxSpeed: 400,
            _nScrollSpeed: 150,
            _nMaxScrollTop: 200,
            _getFandxMaxHeight: function () {
                var _oSelf = this;
                var _oWin = _oSelf._moWin;
            ...
            _fandx: function (_abFold, _afCallBack) {
                var _oSelf = this, _oWin = _oSelf._moWin, _oTop = getTop(), _oFold = _oSelf.S(_ID._sFold),
                    _oExpand = _oSelf.S(_ID._sExpand), _oSubMailDiv = _oSelf.S(_ID._sSubMail);
                if (_abFold == 'auto') {
                    _abFold = !isShow(_oFold);
                }
                if (_abFold) {
                    show(_oFold, true);
                    show(_oFold, false);
                    qmAnimation.play(_oExpand, _oSelf._getFandxSpeedCfg({
                        tween: "Sine",
                        easing: "easeOut",
                        from: _oSelf._getFandxMaxHeight(),
                        to: _oFold.scrollHeight || 48,
                        onaction: function (_anValue) {
                    ...
                    setClass(_oSubMailDiv, "qm_con_expand clearfix");
                    qmAnimation.play(_oExpand, _oSelf._getFandxSpeedCfg({
                        from: _oFold.scrollHeight || 48,
                        to: _oSelf._getFandxMaxHeight(),
                        tween: "Sine",
                        easing: "easeOut",
                        onaction: function (_anValue) {
                            _oExpand.style.height = _anValue + 'px';
                    ...
                    _oSelf._asyncGetSubMail(null, function (_aoContent) {
                        _oSelf._insertHTML(_aoContent, function () {
                            ...
                            if (_oSelf.attr(_aoTarget, "newmail") == "true") {
                                ...
                                hasClass(_aoTarget, "mailunread") && rmClass(_aoTarget, "mailunread");
                                !hasClass(_aoTarget, "mailread") && addClass(_aoTarget, "mailread");
                                setMailRead(false, extend(QMMailList.getCBInfo(_oSelf._moWin, _oSelf.getCMailId_()), {
                                    bNoShowFilter: true,
                                    bNoSendMailMgr: true
                                }));
                            }
            ...
            _loadScrollTop: function (_afCallBack) {
                ...
                qmAnimation.play(_oWin.document.body, _oSelf._getFandxSpeedCfg({
                    win: _oWin,
                    from: _oSelf._getScrollFrom(_nTo),
                    to: _nTo,
                    tween: 'Sine',
                    easing: "easeIn",
                    onaction: function (_anValue) {
            ...
            delMail: function (_aoTarget) {
                ...
                rmMail(_oSelf.attr(_aoTarget, "opt") || 0, extend(_oCBInfo, {
                    oncomplete: function (_aoCBInfo, _aoResult) {
                        _oSelf.dealCustomUI('MailMethod', [_aoCBInfo, 'delSubMail', _aoResult]);
                        _oSelf._fandx(true, function () {
                            removeSelf(_oSelf.S(_ID._sSubMail));
    ...
    function spread(_aoWin, _asRptKey, _aoCfg) {
        ...
        if (_oWin.location && (/^(m\d+\.|set\d\.)?mail.qq.com$/.test(_oWin.location.hostname) || /webdev(\d)*.mail.qq.com$/.test(_oWin.location.hostname))) {
            ...
            var webmail = _oWin.document.getElementById("webmail") || null,
                notWebmail = _oWin.document.getElementById("notWebmail") || null;
            if (webmail) {
