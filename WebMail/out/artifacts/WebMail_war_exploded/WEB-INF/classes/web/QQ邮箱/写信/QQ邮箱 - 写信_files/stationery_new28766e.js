...
QMStationery.prototype._getStationeryInfo = function (_afCallback) {
    var _oTop = getTop(), g_sBaseImageUrl = getTop().getPath("stationery"),
        _oSys = new (_oTop.QMAjaxRequest)(_oTop.T("/cgi-bin/stationerylist?t=stationery_data&sid=$sid$&cmd=all&limit=500&newparam=1").replace({sid: _oTop.getSid()}), "get");
...
QMStationery.prototype._initNewStationery = function () {
    getTop().S('new_stationery', window).innerHTML = getTop().T('<a title="$title_content$" class="$button_gray_f$" onclick="$dosomething$;event.returnValue=false;return false;" href="javascript:void(0);"><img class="design_btn_img" src="$images_path$spacer1e9c5d.gif" />$text$</a>').replace({
        button_gray_f: parseInt(getTop().goUserInfo.get("CUSTOM_STATIONERY_COUNT")) >= 8 || !getTop().getUrlParams(getTop().getMainWin().location)["sid"] ? "button_gray_f disabled" : "button_gray_f",
        dosomething: parseInt(getTop().goUserInfo.get("CUSTOM_STATIONERY_COUNT")) >= 8 || !getTop().getUrlParams(getTop().getMainWin().location)["sid"] ? "" : "QMStationery.getInstance().goToStationSetting();",
        text: parseInt(getTop().goUserInfo.get("CUSTOM_STATIONERY_COUNT")) >= 8 || !getTop().getUrlParams(getTop().getMainWin().location)["sid"] ? "&nbsp;\u8BBE\u8BA1\u65B0\u4FE1\u7EB8nbsp;" : "&nbsp;&nbsp;&nbsp;&nbsp;\u8BBE\u8BA1\u65B0\u4FE1\u7EB8",
        title_content: parseInt(getTop().goUserInfo.get("CUSTOM_STATIONERY_COUNT")) >= 8 || !getTop().getUrlParams(getTop().getMainWin().location)["sid"] ? "\u60A8\u4FDD\u5B58\u7684\u4FE1\u7EB8\u5DF2\u6EE18\u5F20\uFF0C\u8BF7\u5220\u9664\u90E8\u5206\u4FE1\u7EB8\u540E\u518D\u8FDB\u884C\u8BBE\u8BA1" : "",
        images_path: this._msBaseImageUrl
    });
};
QMStationery.prototype.setPersonStationery = function (_asStationeryId, _abSetDefault) {
    var _oTop = getTop(), _oSelf = this;
    var _sUrl = _oTop.T('/cgi-bin/customstationery_mgr?t=getPersonStationery&s=newajax&func=body&stationeryid=$stationeryid$&sid=$sid$&rancode=$rancode$').replace({
        stationeryid: _asStationeryId,
        sid: _oTop.getSid(),
        rancode: Math.random() + (_abSetDefault ? '&setdefault=1' : '')
    });
    var _oXhr = new (_oTop.QMAjaxRequest)(_sUrl, 'get');
    _oXhr.onComplete = function (_aoXml) {
        if (_aoXml) {
            eval('var data = ' + _aoXml.responseText);
            var _sServerName = location.protocol + '//' + location.host,
                _sHeader = data.header.replace(/\/?cgi-bin\/viewfile/, _sServerName + '/cgi-bin/viewfile');
...
QMStationery.prototype.setSysStationery = function (b, c, a) {
    var d = this._getStationery(b), f = d.data ? d.data.htmlheader : "", e = d.data ? d.data.htmlbottom : "",
        g = d.data ? d.data.id : "";
...
QMStationery.prototype._fShowStationery = function (a) {
    ...
    for (var u = q._mnStart; u < Math.min(q._mnStart + q._mnPerPage, s.length); u++) {
        ...
        if (k && k.data) {
            if (k.data.id < 1) {
                l.push(getTop().T(['<li>', '<a href="javascript:void(0);" onclick="QMStationery.getInstance().setBlank();event.returnValue=false;return false;">', '<img src="$baseImageUrl$newicon/misc/xinzhi_icon2s.png" title="$title$"/>', '</a>', '</li>']).replace({
                    baseImageUrl: q._msBaseImageUrl,
                    title: '\u4E0D\u4F7F\u7528\u4FE1\u7EB8'
                }));
            }
            else if (k.data.id < 10000) {
                l.push(q._msSysTmpl.replace({
                    type: u,
                    baseImageUrl: q._msBaseImageUrl,
                    tip: k.data.id,
                    title: k.data.title
                ...
                l.push(q._msPersonTmpl.replace({
                    stationeryId: k.data.id,
                    sid: getTop().getSid(),
                    rancode: Math.random()
                }));
            }
    ...
    var o = r.S("paper_prev_b", window), m = r.S("paper_next_b", window), p = r.S("paper_prev_b_img", window),
        n = r.S("paper_next_b_img", window), f = function (d) {
        ...
        j = [[1, 0, "\u5DF2\u7ECF\u6CA1\u6709\u4E0A\u4E00\u9875\u4E86", "\u4E0B\u4E00\u9875", e, b, 'arrow_tableft_disable', 'arrow_tabright'], [0, 1, "\u4E0A\u4E00\u9875", "\u5DF2\u7ECF\u6CA1\u6709\u4E0B\u4E00\u9875\u4E86", c, e, 'arrow_tableft', 'arrow_tabright_disable'], [0, 0, "\u4E0A\u4E00\u9875", "\u4E0B\u4E00\u9875", c, b, 'arrow_tableft', 'arrow_tabright'], [1, 1, "\u5DF2\u7ECF\u6CA1\u6709\u4E0A\u4E00\u9875\u4E86", "\u5DF2\u7ECF\u6CA1\u6709\u4E0B\u4E00\u9875\u4E86", e, e, 'arrow_tableft_disable', 'arrow_tabright_disable']],
        g = 2;
...
QMStationery.prototype.adjust = function (a) {
    var f = 27 + 6, d = 24, e = 80, j = getTop(), i = j.S('stationery_div', window), h = j.S('quickaddr_div', window),
        b = i.clientHeight, c = this._mnPerPage, g = this._mnPerPage;
...
QMStationery.prototype.resize = function (a) {
    var e = getTop(), d = e.S('stationery_div', window), c = e.calcPos(e.S('quickaddr_div', window), 'json').top,
        b = e.calcPos(d, 'json').top;
    if (e.gnIEVer == 6) {
        b = c;
    }
    else {
