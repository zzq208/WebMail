(function (d, c) {
    var a = {
        ...
        _sMAINFRAMEWEBPUSHTIPTITLE: "mfwebpushtiptitle",
        _nTOTALSERVICES: 31,
        _mnOpenServices: 0,
        _moDataCache: {},
        _moDataTime: {},
            ...
            (function () {
                if (++m < j.length) {
                    ...
                    (l[e] = new k._popuptip({
                        type: e,
                        count: f,
                        content: b[e].replace(g),
                        onclick: k._action(e, h[e]),
                        onload: i
                    })).r = k._moDataTime[e];
                }
            })();
        ...
        _isRefreshWin: function (e) {
            var k = this, m = "cgi-bin/today", l = "cgi-bin/mail_list", f = false,
                i = QMMailList.getCBInfo(getMainWin()), n = getMainWin().location.href, j = getUrlParams(n);
            if (n.indexOf(m) > -1) {
                f = true;
            }
            var g = function (p, o) {
        ...
        _moReloadWinServices: 0,
        _moReloadWinTimeout: null,
        _mbNeedReloadMain: true,
        _reloadWin: function (e, f) {
            var g = this;
        ...
        _setDocTitle: function (f) {
            try {
                ...
                if (f && j.getAttribute(m._sMAINFRAMEWEBPUSHTIPTITLE) != "1") {
                    ...
                    var h = (m._moDataCache[1] || {}).length, i = (m._moDataCache[2] || {}).length,
                        g = (m._moDataCache[4] || {}).length;
                    if (h || i || g) {
    ...
    var b = {
        _TITLE: TE(['\u6536\u5230', '$@$if($email$)$@$ $email$\u5C01\u65B0\u90AE\u4EF6$@$endif$@$', '$@$if($sms$)$@$ $sms$\u6761\u65B0\u77ED\u6D88\u606F$@$endif$@$', '$@$if($plp$)$@$ $plp$\u4E2A\u65B0\u6F02\u6D41\u74F6$@$endif$@$']),
        _CONTAINER: T(['<div id="webpushtipcontainer" class="webpushtipoutter"></div>']),
        ...
        1: TE(['<p class="notify_account">$senderNick$ $@$if($senderEmail$)$@$<span class="addrtitle notify_accountmail">&lt;$senderEmail$&gt;</span>$@$endif$@$</p>', '<p class="notify_title">$@$if($subject$)$@$$subject$$@$else$@$(\u65E0\u4E3B\u9898)$@$endif$@$</p>', '<p class="notify_digest">$summary$</p>']),
        2: TE(['<p class="notify_account">$senderNick$ $@$if($senderEmail$)$@$<span class="addrtitle notify_accountmail">&lt;$senderEmail$&gt;</span>$@$endif$@$</p>', '<p class="notify_digest">$subject$</p>']),
        4: T(['<p class="notify_title">\u60A8\u6536\u5230\u4E00\u4E2A\u65B0\u7684\u6F02\u6D41\u74F6</p>'])
    };
    a._popuptip = function (e) {
        getTop().LogKV({sValue: 'notifytip|pop'});
        this._init(e);
    };
    a._popuptip.prototype = {
        update: function (e) {
            var f = this;
            if (!f._mbClosing) {
                for (var g in e) {
            ...
            f.play({
                speed: "slow", onaction: function (l, k) {
                    if (!j._mbClosing) {
            ...
            f.play({
                speed: "slow", onaction: function (j, i) {
