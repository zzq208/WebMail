...
QMTip._investigate = function (a, c, b) {
    var d = getTop();
    if (a < 10000) {
        d.ossLog(c == "mark" ? "delay" : "realtime", "all", d.TE("stat=tips&type=$type$&tipid=$tipid$$@$if($loc$)$@$&loc=$loc$,,,$tipid$$@$endif$@$").replace({
            tipid: a,
            type: c,
            loc: b
        }));
...
QMTip._animation = function (b, a) {
    try {
        var f = getTop(), d = f.S(b.domid + "tipcontainer", b.win || b.doc),
            c = new (f.qmAnimation)({from: 1, to: 100});
        c.play({
            speed: "slow", onaction: function (h, e) {
...
QMTip._modifyPosition = function (a) {
    var w = getTop(), y = a.win, z = a.domid, u = arguments.callee, o = w.S(z, y, y), m = w.S(z + "rlt", y),
        s = w.S(z + "newtips", y), v = w.S(z + "tipcontainer", y), l = w.bodyScroll(y, 'clientWidth'),
        k = w.bodyScroll(y, 'clientHeight');
    if (!(o && m)) {
        return;
    }
...
QMTip.showMailList = function (a, d, e, b, c) {
    ...
    var k = d.body, l = {
        ...
        doc: d,
        type: 2,
        msg: '<div class="addrtitle">\u5907\u6CE8\u4FE1\u606F\uFF1A</div>' + e,
        arrowOffset: 10,
        icon: "tipsico_note"
    }, m = o.S(q, d);
    o.removeSelf(m);
    o.insertHTML(k, "beforeEnd", QMTip._TEMPLATE._PANEL.replace(l));
    var f = o.bodyScroll(d, 'scrollLeft') + b - 15, g = c, j = o.bodyScroll(d, 'clientWidth'), m = o.S(q, d),
        n = o.S(p + "tipcontainer", d), i = n.offsetWidth, h = n.offsetHeight;
    if (f + i > j) {
        f -= f + i - j + 5;
    }
    if (g - h > 10) {
