(function (_aoUndefined) {
    var _oNetDisk = {}, _oDiskStat = {nNum: 0, nMax: 3, oBound: []}, _oMenuData = {
            "weiyun": {
                sAlias: "weiyun",
                sLabel: "\u5FAE\u4E91",
                sUserid: "",
        ...
        }, _oTop = getTop(), _oProgressFunc = {}, _TIMER_MAX = 30, _TIMER_INTERVAL = 6 * 1000, _nTimerId = 0,
        _nTimerCnt = _TIMER_MAX, _goLocalInfo;
    _oNetDisk.DEBUG = {"account": 0, "connect": 0, "sharelink": 0};
    _oNetDisk.CONST = {
        DATA_NAME: "NetDisk",
        TYPE_WEIYUN: "weiyun",
        TYPE_DROPBOX: "dropbox",
        TYPE_GOOGLEDRIVE: "googledrive",
        TYPE_MOBILE: "mobile",
        TYPE_ALL: ["weiyun", "dropbox", "mobile"]
    };
    ...
    (function (_aoUndefined) {
        ...
        _oDataCenter.sync = function (_asPath, _aoCallbacks, _asForceRemoteRet) {
            var _oPath = _asPath.split(":"), _sType = _oPath[0], _sPath = _oPath[1] || "/", _nTRIGGER = 60,
                _nCurrTimeStamp = this.getCurrTime(), _nFileTimeStamp = this.getTimeStamp(_asPath),
                _bIsPathExist = !!this.getPath(_asPath),
                _sIsRemote = typeof (_asForceRemoteRet) != "undefined" ? _asForceRemoteRet : (!this.getPath(_asPath) ? "0" : (_nCurrTimeStamp - _nFileTimeStamp > _nTRIGGER ? "1" : "0")),
                _Fid = _asPath == "weiyun:/" ? "/" : _oTop.encodeURI(this.getFile(_asPath) && this.getFile(_asPath).sId || ""),
                _sUrl = _oTop.T("/cgi-bin/netdriveapi?sid=$sid$&r=$r$&isremote=$isremote$&action=getfilelist&drivename=$label$&userid=$userid$&path=$path$&id=$fid$&ef=js").replace({
                    sid: _oTop.getSid(),
                    r: Math.random(),
                    isremote: _sIsRemote,
                    label: _sType,
                    userid: this.netDiskMgr.getInfo(_sType).sUserid,
                    fid: _Fid,
                    path: encodeURIComponent(_sPath)
                }), _oCallbacks = _aoCallbacks || {};
            ...
            if (this._moAjax) {
                ...
                this._moAjax.onError = function (_aoXmlObj, _asError) {
                    _oCallbacks.oncomplete && _oCallbacks.oncomplete.apply(_oDataCenter, [false, {
                        ret: -998,
                        msg: this._msErrType || "timeout"
                    }, _oDataCenter._filterPath(_oDataCenter.getPath(_asPath))]);
                    _oDataCenter._moAjax = _aoUndefined;
                };
                this._moAjax.send();
            }
        ...
        _oDataCenter.put = function (_asPath, _aoList, _afChange) {
            ...
            !_oFileCache[_asPath] && (_oFileCache[_asPath] = {
                bDir: true,
                sId: "/",
                sFakeId: "/",
                sPath: "/",
    ...
    (function (_aoUndefined) {
                ...
                rule: function () {
                    ...
                    return {
                        "nd_list": function (_aoEvent, _aoTarget) {
                            ...
                            var _oDomTarget = _oTop.getEventTarget(_aoEvent),
                                _oDomFile = _oDomTarget.tagName == "LABEL" ? _oDomTarget : _oTop.parents("label", _oDomTarget)[0],
                                _oLastFile = this._DOM,
                                _sFileId = _oDomFile.getAttribute("for") || _oDomFile.getAttribute("htmlFor"),
                                _bFileChecked = _oDomTarget.tagName == "INPUT" ? _oTop.S(_sFileId).checked : !_oTop.S(_sFileId).checked,
                                _oDomList = _oTop.parents("ul", _oDomFile)[0];
                            ...
                            setTimeout(function () {
                                var _nCnt = 0, _oDlg = _oTop.QMDialog("netDiskListDlg"),
                                    _oDomSelectList = _oTop.finds("input[type='checkbox']", _oDlg.S("disklist"));
                            ...
                            var _sType = _oTop.QMDialog("netDiskListDlg").S("con").getAttribute("type"),
                                _oSelectedList = [],
                                _oDomSelectList = _oTop.QMDialog("netDiskListDlg").S("disklist").getElementsByTagName("input");
                            for (var i = 0, l = _oDomSelectList.length; i < l; i++) {
                            ...
                            var _oDom = _aoTarget.parentNode,
                                _sType = _oTop.QMDialog("netDiskListDlg").S("con").getAttribute("type"),
                                _sPath = _oTop.parents("li[path]", _aoTarget)[0].getAttribute("path");
        ...
        _oNetDiskView._route = {
            "clsBind": function () {
                ...
                var _oDoc = _aoDomTarget.ownerDocument, _oNetDiskInfo = _oNetDisk.getInfo(),
                    _oTips = _oNetDiskInfo.oTips;
                if (!_oTips) {
                    return;
                }
                _oTop.waitFor(function () {
                    ...
                    if (_abIsOk) {
                        var _oPos = _aoOffsetPos || {}, _nLeft = +(_oPos.offsetLeft || 0),
                            _nTop = +(_oPos.offsetTop || 0), _oTip = _oTips[_asTipType];
                        debug("tip id " + _oTip.sId + ":" + _oTop.QMTip.getStatus(_oTip.sId));
                        if (_oTip && !_oTip.bIsShowed && _oTop.QMTip.getStatus(_oTip.sId) != "showed") {
                            debug("cb show!");
                            _oTop.QMTip.show({
                                tipid: _oTip.sId,
                                domid: _aoDomTarget.id,
                                win: _oDoc.defaultView || _oDoc.parentWindow,
                                msg: _oTip.sMsg,
                                arrow_direction: _oTip.sArrowType,
                                arrow_offset: 25 + _nLeft,
                                height_offset: 4 + _nTop,
                                tip_offset: -90 + _nLeft,
                                width: 305,
                                notlog: false,
                                bForceShow: true
                            });
                ...
                var _oDoc = _aoDom.ownerDocument, _sMenuId = "netdisk_droplist", _oPos = _oTop.calcPos(_aoDom, "json"),
                    _oNetDisk = this.netDiskMgr, _oNetDiskInfo = _oNetDisk.getInfo(),
                    _oMainFramePos = _oTop.calcPos(_oTop.S("mainFrame"), "json"),
                    _oMenuItems = _oTmpl["bindDisk"].replace({
                        sid: _oTop.getSid(),
                        from: "compose",
                        info: [_oNetDisk.getInfo("weiyun") && _oTop.extend({images_path: _oTop.getPath("image")}, _oNetDisk.getInfo("weiyun")) || {images_path: _oTop.getPath("image")}]
                    });
                new _oTop.QMDialog({
                    sId: _sMenuId,
                    sTitle: "\u4ECE\u7F51\u76D8\u9009\u62E9",
                    nWidth: 400,
                    sBodyHtml: _oMenuItems,
                    onshow: function () {
                ...
                new _oTop.QMDialog({
                    ...
                    nWidth: 430,
                    nHeight: 282,
                    sBodyHtml: _oTmpl["bindDisk"].replace({
                        sid: _oTop.getSid(),
                        from: _asFrom,
                        info: [_oNetDisk.getInfo("weiyun") && _oTop.extend({images_path: _oTop.getPath("image")}, _oNetDisk.getInfo("weiyun")) || {images_path: _oTop.getPath("image")}]
                    })
                ...
                _oTop.confirmBox({
                    title: "\u89E3\u9664\u7ED1\u5B9A",
                    msg: _oTop.T("\u89E3\u9664\u7ED1\u5B9A$label$?").replace({label: _oNetDiskInfo.sLabel}),
                    width: 450,
                ...
                new _oTop.QMDialog({
                    sId: "netDiskListDlg",
                    sTitle: [_oNetDiskInfo.sAlias == "weiyun" ? '<span class="dialog_icon icon_weiyun"></span>' : '<span class="dialog_icon icon_dropbox"></span>', "\u4ECE", _oNetDiskInfo.sLabel, "\u9009\u62E9\u6587\u4EF6"].join(""),
                    nWidth: 430,
                    nHeight: 440,
                    sBodyHtml: _oTmpl["container"].replace({sType: _asType, sPath: "/"}),
                    sClassName: "p-otherdisk",
                    onshow: function () {
                        var _sPath = this.S("disklist").getAttribute("path"),
                            _sType = this.S("con").getAttribute("type");
                ...
                new _oTop.QMDialog({
                    ...
                    nWidth: 430,
                    nHeight: 280,
                    sBodyHtml: _oTmpl["errTmpl"].replace({
                        sId: _oNetDiskInfo.sId,
                        sType: _asType,
                        sPath: _asPath,
                        sLabel: _oNetDiskInfo.sLabel,
                        sErrorType: _asError
                    }),
                    sClassName: "p-otherdisk",
                    onshow: function () {
        ...
        _oNetDiskView._enter = function (_aoDom, _asType, _asPath, _abIsRetry) {
            var _bRoot = _asPath == "/";
            _oNetDiskView.netDiskMgr.dataCenter.sync([_asType, _asPath].join(":"), {
                onready: function (_aoList) {
                    if (!_aoList) {
        ...
        _oNetDiskView._listError = function (_aoDomTarget, _asType, _asPath, _abRoot) {
            _oNetDiskView._list(_aoDomTarget, _oTmpl[_abRoot ? "rootError" : "listError"].replace({
                bIsLeaf: !_abRoot,
                sType: _asType,
                sPath: _asPath
            }), true);
        ...
        _oNetDiskView._listLoading = function (_aoDomTarget, _abIsRoot) {
            var _oDomContainer = _oTop.QMDialog("netDiskListDlg").S("disklist"),
                _oDomLoading = _oTop.finds("ul[ui-type='loading']", _oDomContainer);
            ...
            queue: {
                Q: [], E: [], _put: function (_aoQueue) {
                    ...
                    if (!_oSelf._bRunning) {
                        ...
                        var _oQueryData = _oList.oQueryData, _oCallbacks = _oList.oCallbacks || {},
                            _oNetDiskInfo = _oNetDisk.getInfo(_oQueryData[0].sType),
                            _oQueryStr = _oTop.TE(['sid=$sid$&r=$r$&action=getshareurl&ef=js&drivename=$alias$&userid=$userid$&resp_charset=UTF8', '$@$for($data$)$@$', '&id=$@$eval encodeURIComponent($sId$)$@$&path=$@$eval encodeURIComponent($sPath$)$@$&name=$@$eval encodeURIComponent($sName$)$@$&pid=$@$eval encodeURIComponent($sPid$)$@$&size=$nSize$', '$@$endfor$@$']);
                        _oSelf._bRunning = true;
                        _oCallbacks.onready && _oCallbacks.onready.call(_oNetDisk, _oQueryData);
                        _oTop.QMAjax.send("/cgi-bin/netdriveapi", {
                            method: "POST",
                            timeout: "10000",
                            content: _oQueryStr.replace({
                                sid: _oTop.getSid(),
                                alias: _oNetDiskInfo.sAlias,
                                userid: _oNetDiskInfo.sUserid,
                                data: _oQueryData
                            }),
                            onload: function (_abIsOk, _asParam, _aoXhr) {
                ...
                var _nBatchNum = _BATCH_NUM, _oQueue = [], _oCallbacks = _aoCallbacks || {},
                    _oList = _oTop.isArr(_aoFileList) ? _aoFileList : [_aoFileList],
                    _oNetDiskInfo = _oNetDisk.getInfo(_oList[0].sType);
    ...
    _oNetDisk.syncDiskInfo = function (_afComplete) {
        _syncReset(true);
        _oTop.QMAjax.send(_oTop.T("/cgi-bin/netdriveapi?sid=$sid$&r=$r$&action=getauthlist&ef=js").replace({sid: _oTop.getSid()}), {
            method: "GET", timeout: 2000, onload: function (_abIsOk, _asParam) {
                if (_abIsOk) {
    ...
    _oNetDisk.removeDisk = function (_avParam, _afComplete, _abShowTips) {
        var _bShowTips = typeof (_abShowTips) == "boolean" ? _abShowTips : true,
            _oNetDiskInfo = typeof (_avParam) == "object" ? _avParam : this.getInfo(_avParam),
            _sType = _oNetDiskInfo.sAlias,
            _sUrl = _oTop.T('/cgi-bin/netdriveapi?sid=$sid$&r=$r$&action=delauthinfo&drivename=$alias$&userid=$userid$&ef=js').replace({
                sid: _oTop.getSid(),
                alias: _oNetDiskInfo.sAlias,
                userid: _oNetDiskInfo.sUserid
            });
    ...
    _oNetDisk.progress = function (_asTaskid, _afProgress, _asActionType) {
        var _nIntervalTime = 1000, _sActionType = typeof (_asActionType) == "undefined" ? "download" : _asActionType,
            _nCurrCnt = 0, _nDesPercent = 12, _nMaxCnt = 3, _nLastPercent = 0,
            _fGetFixPercent = function (_anCurrPercent) {
                var _sSymbol = ((Math.random() + "").split("").pop() % 2 == 0) ? "-" : "+",
                    _sNum = _sSymbol + (+new Date() % 2);
        ...
        (function (_asUrl, _asTaskid) {
            ...
            _oTop.QMAjax.send(_asUrl, {
                method: "GET", onload: function (_abIsOk, _asParam, _aoXhr) {
                    var _oData = {};
                    if (_abIsOk) {
                        try {
                            _oData = eval(["(", _asParam, ")"].join(""));
        ...
        })(_oTop.T('/cgi-bin/netdriveupload?sid=$sid$&r=$r$&action=$action$&taskid=$taskid$&ef=js').replace({
            action: _sActionType == "upload" ? "queryupload" : "querydownload",
            sid: _oTop.getSid(),
            taskid: _asTaskid
        }), _asTaskid);
    };
    _oNetDisk.uploadEntry = function (_aoDom) {
        var _oDiskBound = this.getInfo().oBound, _oItemList = [], _sAttid = _aoDom.getAttribute("attid"),
            _isBigAttach = _aoDom.getAttribute("bigattch") == "true", _sKey = _aoDom.getAttribute("k"),
            _sftnCode = _aoDom.getAttribute("ftncode"), _sftnSaveUrl = _aoDom.getAttribute("ftnsaveurl"),
            _sFid = _aoDom.getAttribute("fid");
    ...
    _oNetDisk.bind = function (_asType, _afCallback) {
        if (_asType == "weiyun") {
            _oTop.QMAjax.send(_oTop.T("/cgi-bin/netdriveback?sid=$sid$&ptype=weiyun&ef=js").replace({sid: _oTop.getSid()}), {
                method: "GET", onload: function (_abIsOk, _asParam) {
                    var _oData = {};
                    if (_abIsOk) {
                        try {
            ...
            _oTop.open(_oTop.T("$base_domain$/cgi-bin/netdriveback?sid=$sid$&ktype=getrequest&ptype=$type$").replace({
                base_domain: _oTop.getTopHost(),
                sid: _oTop.getSid(),
                type: _asType
            }));
    ...
    _oNetDisk.upload = function () {
        var _oInfo = this.getInfo(), _oArgs = arguments, _fUpload = function (_asType, _asUserid, _asAttachId) {
            ...
            var _oNewWin = window.open(_oTop.T(["$base_domain$/cgi-bin/readtemplate", "?action=addtaskupload&userid=$userid$&sid=$sid$&ptype=$type$&mailattach=$attid$&t=attachfolder_page_todisk", "&filename=$filename$&fileicon=$fileicon$&nocheckframe=true"]).replace({
                base_domain: getTop().getTopHost(),
                sid: _oTop.getSid(),
                type: _asType,
                userid: _asUserid,
                attid: _asAttachId,
                fileicon: _oNetDisk.dataCenter.getFileIcon(_sFileName.split(".").pop()).replace("/ico_min/", "/ico_max/").replace("/ico_mid/", "/ico_max/"),
                filename: _sFileName
            }));
        }, _fFtnStorefile = function (_asKey, _asCode, _asFtnSaveUrl) {
            var _sFtnSaveUrl = _asFtnSaveUrl || _oTop.T("/cgi-bin/ftnStoreFile?k=$key$&code=$code$&sid=$sid$&action=save&t=autosave").replace({
                sid: _oTop.getSid(),
                key: _asKey,
                code: _asCode
            });
            ...
            var _sFileName = _asAttachId.split("|").pop();
            var _oNewWin = window.open(_oTop.T(["$base_domain$/cgi-bin/readtemplate", "?&sid=$sid$&ptype=$type$&t=attachfolder_page_todisk", "&fid=$fid$&key=$key$&fcode=$fcode$", "&filename=$filename$&fileicon=$fileicon$&nocheckframe=true", "&action=bigattweiyun"]).replace({
                fcode: _asftnCode || "",
                base_domain: _oTop.getTopHost(),
                sid: _oTop.getSid(),
                type: "weiyun",
                ...
                key: _asKey,
                fileicon: _oNetDisk.dataCenter.getFileIcon(_sFileName.split(".").pop()).replace("/ico_min/", "/ico_max/").replace("/ico_mid/", "/ico_max/"),
                filename: encodeURI(_sFileName)
            }));
        }, _fCheckFile = function (_aoFileInfo) {
            var _sName = _aoFileInfo.sFileName.toLowerCase(), _nSize = parseFloat(_aoFileInfo.sSize),
                _nInvalidFormat = {};
        ...
        if (typeof (_oArgs[0]) != "string") {
            var _oDiskBound = _oInfo.oBound, _oItemList = [], _oDom = _oArgs[0],
                _oWin = _oDom.ownerDocument.defaultView || _oDom.ownerDocument.parentWindow;
                        ...
                        if (_asId == "weiyun" || _asId == "dropbox") {
                            if (_oDiskBound[_asId]) {
                                ...
                                _oNetDisk.bind(_asId, function (_aoData) {
                                    _oTop.confirmBox({
                                        title: "\u63D0\u793A",
                                        msg: _oTop.T("\u5DF2\u6210\u529F\u7ED1\u5B9A\u7F51\u76D8\uFF0C\u70B9\u51FB\u201C\u4E0B\u4E00\u6B65\u201D\u8FDB\u884C\u6587\u4EF6\u8F6C\u5B58\u3002").replace(_aoData.oBound[_asId]),
                                        confirmBtnTxt: "\u4E0B\u4E00\u6B65",
                                        cancelBtnTxt: "\u53D6\u6D88",
                                        width: 400,
                                        onreturn: function (_aIsOk) {
                            ...
                            if (_asId == "ftn") {
                                ...
                                _oMenu.close();
                                var _oAttachItem = parents("div.attachitem", _oDom)[0],
    ...
    _oNetDisk.addTask = function (_asTaskType, _asDiskType, _asPath, _aoCallbacks) {
        var _oCallbacks = _aoCallbacks || {}, _oNetDiskInfo = this.getInfo(_asDiskType),
            _sUrl = _oTop.T(['/cgi-bin/netdriveupload?', 'sid=$sid$&r=$r$&action=$task$&ptype=$alias$&path=$path$', '&mailattach=$attid$&fileid=$fid$&userid=$userid$&ef=js']).replace({
                sid: _oTop.getSid(),
                task: _asTaskType == "download" ? "addtaskdownload" : "addtaskupload",
                path: _asTaskType == "download" ? encodeURIComponent(_asPath) : "",
                attid: _asTaskType == "upload" ? _asPath : "",
                fid: _asTaskType == "download" ? _oTop.encodeURI(this.dataCenter.getFile([_asDiskType, _asPath].join(":")).sId || "") : "",
                alias: _oNetDiskInfo.sAlias,
                userid: _oNetDiskInfo.sUserid
            });
