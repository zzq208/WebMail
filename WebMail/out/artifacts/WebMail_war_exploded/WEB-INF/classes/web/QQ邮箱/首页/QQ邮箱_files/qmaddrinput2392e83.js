    ...
    _setData: function (a) {
        ...
        if (_anType) {
            ...
            _oSelf._mnDelayTimeout = setTimeout(function () {
                QMAjax.send("/cgi-bin/laddr_domain_check", {
                    method: "POST",
                    content: T('sid=$sid$&addrs=$addrs$&addrlist=$list$&acttype=$type$&t=laddr_domain_check&addrfilt=$addrfilt$').replace({
                        sid: getSid(),
                        type: _anType & 3,
                        list: encodeURI(_sCheckAddress),
                        addrfilt: encodeURI(_asAddrs),
                        addrs: encodeURI("a@" + _oCheckDomains.join(";a@"))
                    }),
                    onload: function (_abIsOk, _asParam) {
                        if (_abIsOk && _asParam.indexOf('success:true') > 0) {
                            _oSelf._setData(eval(_asParam));
                            callBack.call(_oSelf, _afCallBack, [{
                                bCksp: _oSelf.permit(_oAddrData[1]),
                                oCklm: _oSelf.getAttachLimit(_oAddrData[0]),
                                oCkdm: _oSelf._getTip(_oAddrData[0])
                            }]);
                        }
                    }
                });
...
QMAddrDomainCheck2.prototype._ui.prototype = {
    check: function () {
...
QMAddrInput.prototype = {
    add: function (b, a, c) {
                ...
                switch (a) {
                    ...
                    case "json":
                        d.push({
                            ...
                            valid: g._bIsValid,
                            isQQ: g._bIsQQAddr,
                            format: h ? ['"', encodeNick(h), '"<', f, '>'].join("") : f
                        });
                        break;
                    case "validemail":
                        if (g._bIsValid) {
        ...
        var l = this, i = l._moAddrMap, h = l._moAddrList, n = l._moTextContainer, k = n.previousSibling,
            g = (k ? l._getAddrPos(k.getAttribute("addr")) : -1) + 1;
        if (i[b]) {
            l._deleteItem(b);
        }
        {
            ...
            insertHTML(n, "beforeBegin", m._ADDRITEM.replace({
                nick: d && e(d, l._moConst._nMaxNickAsiiLen),
                alias: _sAlias && f(_sAlias, l._moConst._nMaxAddrAsiiLen),
                domain: q && htmlEncode(q),
                addr: o,
                css: l._moStyle,
                dispmode: l._msDispMode,
                splitchar: l._msSplitChar,
                isvalid: a,
                isdomainerr: !!l._moErrorDomainMap[q],
        ...
        if (this._mnTextWidthCache != g) {
            ...
            var i = h._moTextContainer, b = h._moConst._nBasicWidth, f = h._moConst._nStepWidth,
                e = g < b ? 0 : 1 + Math.floor((g - b) / f),
                c = Math.min(b + f * e, Math.max(h._moContainer.offsetWidth - 5, 0)), d = i.clientWidth;
            if (c != d) {
                i.style.width = c + "px";
                if (a == "paste") {
                    i.scrollLeft = 0;
        ...
        if (e.selection) {
            ...
            try {
                ...
                var b = i.compareBoundaryPoints(Range.START_TO_nStart, g) == 1,
                    a = i.compareBoundaryPoints(Range.END_TO_nEnd, g) == -1, c = !b && !a;
            }
            catch (f) {
                var c = true;
            }
        ...
        try {
            ...
            d._mnMaxItemView = a.maxItemView || 15, d._mnMinWidth = a.minWidth || 220, d._mnWidth = a.width || "auto", d._moStyle = extend({
                text: "addr_text",
            ...
            d._mEvent = {
                _onfocus: a.onfocus,
                _onblur: a.onblur,
                _onchange: a.onchange,
                _onDel: a.onDel,
                _onkeydown: a.onkeydown,
                _onNotify: a.onNotify
            };
        ...
        a._moAutoComplete = new QMAutoComplete({
            oInput: a._moTextCtrl,
            oPosObj: a._moContainer,
            nMaxItemView: a._mnMaxItemView,
            nMinWidth: a._mnMinWidth,
            nWidth: a._mnWidth,
            ongetdata: function (b) {
        ...
        this._addEventForDom(a, {
            "click": this._doAddrItemClick,
            "dblclick": this._doAddrItemDblclick,
            "mouseover": this._doAddrItemMouseOver,
            "mouseout": this._doAddrItemMouseOut
        }, b);
        ...
        var b = this;
        b._addEventForDom(b._moContainer, {
            "mousedown": b._doContainerMouseDown,
            "selectstart": b._doContainerSelectStart,
            "contextmenu": b._doContainerContextMenu
        });
        var c = {
            "focus": b._doTextFocus,
            "blur": b._doTextBlur,
            "keydown": b._doTextKeyDown,
            "keyup": b._doTextKeyUp,
            "paste": b._doTextPaste
        };
        var a = {
            "focus": b._doSelectFocus,
            "blur": b._doSelectCancel,
            "keydown": b._doSelectKeyDown,
            "keyup": b._doSelectKeyUp,
            "paste": b._doSelectPaste,
            "cut": b._doSelectCut
        };
        if (gbIsTT) {
        ...
        d.unselectable = "on";
        d.style.cursor = "text";
        d.innerHTML = g._moTemplate._INPUT.replace({
            css: g._moStyle.text,
            width: g._moConst._nBasicWidth,
            defaulttext: g._msDefaultText,
        ...
        E(g._moConst._oMenuItems, function (k) {
            var l = k.split("|");
            f.push((l[0] == "seperater" ? g._moTemplate._CONTEXTMENU_SEP : g._moTemplate._CONTEXTMENU_ITEM.replace({
                operate: l[0],
                name: l[1],
                shortcut: T(l[2] || "").replace(g._getCtrlKeyName())
            })));
        });
        insertHTML(c, "afterBegin", g._moTemplate._CONTEXTMENU.replace({
            items: f.join(""),
            width: gbIsMac ? 160 : 130
        }));
        ...
        var i = this._moWindow.document.body, e = i.clientWidth, d = i.clientHeight, j = this._moContextMenu,
        ...
        if (!c || this._isInContainer(a)) {
            ...
            if (p > 0) {
                var q = this._moAddrItemCalcInfo, l = a.clientX - q._nOffsetLeft, m = a.clientY - q._nOffsetTop,
                    n = this._getPointRow(a.clientY), u, o, d, e, s, k, j, g, h, r, f;
                if (n < 0) {
                    n = 0;
                }
                else if (n >= p) {
        ...
        var k = this, l = this._moWindow, f = l.document, g = l.captureEvents ? l : f.body, d = b.clientX,
            e = b.clientY, h = f.createElement("div"), j = f.createElement("div");
        ...
        var i = this._moContainerMouseDownInfo = {
            _bIsInitlize: false, _oInsertCursor: h, _oMover: j, _oContainer: a, _fDoMouseUp: function (m) {
                if (k._moContainerMouseDownInfo) {
                    ...
                    try {
                        if (i._bIsInitlize) {
                            var t = k._moSelectCtrl.value, p = i._oObjectList[i._nInObjIdx]._oInstance,
                                o = p._moContainer, s = [], r = k._moAddrList, q;
                            if (h.parentNode == o) {
        ...
        if (g && g._bIsInitlize) {
            var f = g._oInsertCursor, h = g._oMover, n = g._oObjectList, b = g._nInObjIdx,
                e = n[b]._oInstance._moContainer, l;
        ...
        var n = a.childNodes, q = n[1].offsetParent, l = calcPos(q), k = l[3], m = l[0], g = b.clientX, h = b.clientY,
            c = false;
        ...
        var e = this._moContainerMouseDownInfo = {
            _bIsInitlize: true, _fDoMouseUp: function (j) {
        ...
        switch (a.keyCode) {
            ...
            case 46:
                ...
                break;
            case 8:
                var v = this._getTextSelectionState(), t = this._moTextContainer.previousSibling,
                    z = t && t.getAttribute("addr");
            ...
            case 37:
                ...
                break;
            case 39:
                var v = this._getTextSelectionState(), p = this._moTextContainer.nextSibling,
                    y = p && p.getAttribute("addr");
                ...
                if (!this._moAutoComplete.isShow()) {
                    ...
                    if (_nAddrPos != -1) {
                        ...
                        for (var A = _nAddrPos, o = l.length, f = b ? -1 : o + 1, k = b ? -1 : 1; A != f; A += k) {
                            if (A == o) {
                                var x = "", w = _oAddrMap[l[o - 1]]._oDom, m = h + w.offsetLeft + w.offsetWidth,
                                    n = j + w.offsetTop, e = Math.abs(n - _nTextTop);
                            }
                            else {
                                var x = l[A], w = _oAddrMap[x]._oDom, m = h + w.offsetLeft, n = j + w.offsetTop,
                                    e = Math.abs(n - _nTextTop);
                            }
...
QMAddrInput._CONST = {
    _nCacheId: "QMAddrInput_aTGdf$#HAsGf",
    _nMaxNickAsiiLen: 25,
    _nMaxAddrAsiiLen: 40,
    _nBasicWidth: 13,
    _nStepWidth: 1,
    _oMenuItems: ["cut|\u526A\u5207|$cmd$+X", "copy|\u590D\u5236|$cmd$+C", "paste|\u7C98\u8D34|$cmd$+V", "edit|\u7F16\u8F91", "delete|\u5220\u9664", "seperater", "selectall|\u5168\u9009|$cmd$+A"]
};
...
QMAddrInput._TEMPLATE = {
    _FOLD_ADDR: TE(['<div id="qmAddrInputFold" class="smart_contact_info_wrap" >', '<span class="smart_contact_info">\u5171<font id="qmAddrInputFoldCnt">$addrCnt$</font>\u4E2A</span>', '<span class="smart_contact_gd smart_contact_gd1"></span><span class="smart_contact_gd smart_contact_gd2"></span><span class="smart_contact_gd smart_contact_gd3"></span>', '</div>']),
    ...
    _MOVER: T(['<div style="width:8px;height:8px;*width:12px;*height:12px;', 'font-size:1px;border:2px solid #7B7D84;position:absolute;top:18px;left:9px;"></div>', '<div style="background:#fff;width:11px;height:11px;font:1px;opacity:0;filter:alpha(opacity:0);"></div>']),
    _CONTEXTMENU: T(['<table style="position:absolute;z-index:9999;left:-200px;top:0;"', ' cellspacing="0" cellpadding="0" onmousedown="return false;" unselectable="on">', '<tr><td>', '<div style="border:1px solid #ACA899;width:$width$px;padding:2px;background:#fff;">', '$items$', '</div>', '</td><td style="height:100%;filter:alpha(opacity=50);opacity:0.5;">', '<div style="margin-top:4px;border-left:2px solid black;height:100%;"></div>', '</td></tr>', '<tr><td style="filter:alpha(opacity=50);opacity:0.5;">', '<div style="margin-left:4px;border-top:2px solid black;height:2px;"></div>', '</td><td style="filter:alpha(opacity=50);opacity:0.5;">', '<div style="margin-left:0px;border-top:2px solid black;height:2px;"></div>', '</td></tr>', '</table>']),
    _CONTEXTMENU_ITEM: T(['<div class="menu_item" onmouseover="', 'if (this.className == \x27menu_item\x27)', '{', 'this.className = \x27menu_item_high\x27;', '}', '" onmouseout="', 'if (this.className == \x27menu_item_high\x27)', '{', 'this.className = \x27menu_item\x27;', '}', '" style="padding:0 20px;line-height:19px;" unselectable="on" opt="$operate$">', '<div style="float:right;" unselectable="on" opt="$operate$">$shortcut$</div>', '$name$', '</div>']),
    _CONTEXTMENU_SEP: T(['<div style="font-size:1px;height:7px;overflow:hidden;" unselectable="on">', '<div style="border-top:1px solid #ACA899;margin-top:3px;"></div>', '</div>']),
    _ADDREDIT: TE(['$@$if($nick$)$@$"$nick$"<$@$endif$@$', '$addr$', '$@$if($nick$)$@$>$@$endif$@$', '$splitchar$']),
    _ADDRDISP: T(['$alias$', '$@$if($domain$)$@$', '<span class="domain" unselectable="on" addr="$addr$">@$domain$</span>', '$@$endif$@$'])
};
...
QMAddrInput._isShowTip = function () {
    ...
    if (typeof b == "undefined") {
        ...
        QMAjax.send("/cgi-bin/readtemplate", {
            content: T("sid=$sid$&t=tip&s=isshowtip&tipid=29").replace({sid: getSid()}), onload: function (c, d) {
