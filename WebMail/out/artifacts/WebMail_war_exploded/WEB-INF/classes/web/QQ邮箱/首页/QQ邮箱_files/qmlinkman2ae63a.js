...
QMLinkman.prototype = {
    _init: function () {
        ...
        var r = this, l = r._moConfig, u = getTop(), h = a.nTime, t = QMLinkman._TEMPLATE, w = a._oGroup.id,
            j = u.QMAddress, m = j.getGroup(w), k = m.addressesId, o = [], v = l.onfilteraddr;
        ...
        if (n) {
            for (var d = 0, g = n.length; d < g; d++) {
                var s = j.getGroup(n[d]), f = s.addressesId.length,
                    b = l.bSupportGroup && f > 0 && f < 21 && n[d] > 9 && j.getAncestor(s) == 1;
                o.push({
                    sType: 'group',
                    sItemValue: t._caption.replace({
                        other: b || (n[d] === 3 && l.nType === 0) ? t._groupRightItem.replace({
                            disp: 'none',
                            name: n[d] === 3 ? '\u6E05\u7A7A' : '\u5168\u9009',
                            title: n[d] === 3 ? '\u6E05\u7A7A\u6700\u8FD1\u8054\u7CFB\u4EBA\u8BB0\u5F55' : '',
                            fun: 'g' + s.id
                        }) : '', cac: b ? 'lm_cac' : '', name: s.name
                    }),
                    sHashCode: 'g' + s.id,
                    nTime: j.getTime(),
                    _oGroup: s
        ...
        for (var c = 0, e = k.length; c < e; c++) {
            ...
            i && (!v || u.callBack(v, [i]) !== false) && o.push({
                sType: 'item',
                sItemValue: t._left_item.replace({
                    beforeImage: i.icon ? t._addrIcon.replace(i) : (b && i.nShortcutGroupId ? t._virtualGroupAccountIcon : (i.qq == getTop().g_admuin ? t._domainMailGroupAccountIcon : '')),
                    title: i.email,
                    name: i.name,
                    memo: i.memo
                }),
                sHashCode: i.id,
                nTime: j.getTime(),
                sDisplay: r._moPosition[i.email] ? 'none' : ''
            });
        ...
        if (typeof m.moreChildId != "undefined") {
            ...
            o.push({
                sType: 'moreItem',
                sItemValue: t._left_item.replace({title: q.name, name: "<b>\u663E\u793A\u66F4\u591A\u2193</b>"}),
                sHashCode: 'm' + q.id,
                nTime: j.getTime(),
                _oGroup: q
            });
        ...
        var j = this, e = j._moConfig, f = e.oContainer, d = getTop().QMAddress, k = QMLinkman._TEMPLATE,
            h = e.oMainGroup, g = [];
        ...
        for (var b = 0, c = h.length; b < c; b++) {
            ...
            g.push({
                ...
                sHashCode: 't' + i.id,
                _oGroup: i,
                sItemValue: k._title.replace({title: i.name, fun: "g" + i.id, name: i.id == 3 ? "\u6E05\u7A7A" : ""})
            });
        }
        var a;
        j._moTreeView = new QMTreeView({
            oContainer: S(j._msPid + 'tree', j._moDoc), oDefaultItem: g, ongetdata: function (l) {
                    ...
                    if (s === "3") {
                        confirmBox({
                            ...
                            onreturn: function (t) {
                                ...
                                QMAjax.send(T("/cgi-bin/laddr_opr?sid=$sid$&action=clearupdatetime").replace({sid: getSid()}), {
                                    onload: function (u, v) {
                                        if (u && v.indexOf('<!--cgi exception-->') == -1) {
                                            ...
                                            if (w) {
                                                j._moTreeView.findSub(w).innerHTML = k._nobody.replace({
                                                    locale: getTop().getLocale(),
                                                    message: "\u6700\u8FD1\u8054\u7CFB\u4EBA"
                                                });
                                            }
        ...
        var f = this, h = S(f._msPid + 'tree', f._moDoc), d = S(f._msPid + 'hint', f._moDoc), g = QMLinkman._TEMPLATE,
            c = 10;
        if (!a || !a.length) {
            b(d, false);
            return;
        ...
        function b(l, i) {
            ...
            m ? m[i ? 'expand' : 'fold'](l, {
                durlimit: 300, type: 'wait', speed: 'fast', onready: function () {
        ...
        var g = this, a = function (j, k) {
                ...
                return null;
            }, e = S(g._msPid + 'search', g._moDoc), f = S(g._msPid + 'searchtext', g._moDoc),
            c = S(g._msPid + 'cancel', g._moDoc), h = "rgb(160, 160, 160)", i = "\u67E5\u627E\u8054\u7CFB\u4EBA...";
        ...
        var k = this, g = getAddrACData(a, null, k._moConfig.onfilteraddr || k._moConfig.bSupportGroup || c),
            h = ['<div class="groupopen">', '<div class="settingtable qqshowbd" style="padding:4px 0 3px 8px;cursor:default;">', '\u67E5\u627E\u5230\uFF1A', '</div>', '</div>'],
            l = QMLinkman._TEMPLATE, i = /\"(.*)\" &lt;(.*)&gt;$/, p = S(k._msPid + 'search', k._moDoc);
        if (!p) {
            return;
        }
        k._mnSearchCount = 0;
        for (var d = 0, e = g.length; d < e; d++) {
            ...
            if (f.oAddress.nShortcutGroupId || f.oAddress.qq == getTop().g_admuin) {
                h.push(l._research_item.replace({
                    pid: k._msPid,
                    id: f.oAddress.id,
                    title: [f.oAddress.name, ' (', f.oAddress.email, ')'].join(""),
                    disp: k._moPosition[m] ? "none" : "",
                    name: f.sItemValue,
                    email: f.oAddress.email
                }));
            }
            else {
                var j = /\"(.*)\" &lt;(.*)&gt;\((.*)\)$/;
        ...
        var e = this, f = S(e._msPid + 'tree', e._moDoc), c = S(e._msPid + 'hint', e._moDoc),
            d = S(e._msPid + 'search', e._moDoc), b = S(e._msPid + 'cancel', e._moDoc);
        show(f, !a);
        show(c, !a);
        show(d, a);
        setClass(b, a ? "lm_sclose" : "lm_sopen");
    }, _clickItem: function (a, b) {
        var m = this, k = getTop().QMAddress, f = k.getAddress(b, true) || m._moCustomAddrs[b], g = m._moConfig, l = [],
            n, h;
        if (g.nType == 0) {
            l = [f];
        }
        ...
        var j = this, k = getTop(), c = k.document.location.href.indexOf('from_wm_newwin') > -1,
            b = !c && k.document.location.href.indexOf('t=newwin_frame') > -1, g = j._moConfig.oContainer,
            f = g && k.calcPos(g, 'json').top, h = k.finds('.lm_panel', g) && k.finds('.lm_panel', g)[0],
            d = h && k.calcPos(h, 'json').top;
...
QMLinkman._TEMPLATE = {
    _title: T('<div class="settingtable qqshowbd" style="padding:4px 0 3px 8px;cursor:default;">' + '<a class="lm_rt" fun="$fun$">$name$</a>$other$$title$</div>'),
    _caption: TE(['<div class="$cac$" onmouseover="this.style.background=\'#FFF6DF\'" onmouseout="this.style.background=\'#fff\'" style="padding:2px 0 1px;overflow:hidden;height:100%;">', '<div style="overflow:hidden;width:100%">', '$other$', '<div title="$@$eval htmlEncode($name$)$@$" class="lm_ca">', '<a nocheck="true"><input type="button" class="lm_ico"/>$@$eval htmlEncode($name$)$@$</a>', '</div>', '</div>', '</div>']),
    _groupRightItem: T('<a class="lm_rt" fun="$fun$" title="$title$">$name$</a>'),
    _plane: TE(['<div class="lm_sbar">', '<input type="button" id="$pid$cancel" class="lm_sopen"/>', '<div>', '<input id="$pid$searchtext" type="text" class="txt" value="\u67E5\u627E\u8054\u7CFB\u4EBA..." autocomplete="off"/>', '</div>', '</div>', '<div class="lm_panel">', '$@$if($fromaddr$)$@$', '<div class="groupopen"><div class="settingtable qqshowbd" style="padding:4px 0 3px 8px;cursor:default;">\u539F\u90AE\u4EF6\u7684\u53D1\u4EF6\u4EBA</div></div>', '<a nocheck="true" title="$fromaddr$" class="lm_addr " style="margin: 5px 0;" id="orgsender">$fromname$</a>', '$@$endif$@$', '<div id="$pid$tree" ></div>', '<div id="$pid$search"  style="display:none;"></div>', '</div>']),
    _hint: TE(['<ul class="addr_associate">', '<li class="fn" style="padding:4px 0 4px 8px; margin:0; height:15px; line-height:15px;" >\u60A8\u662F\u5426\u8FD8\u8981\u627E...</li>', '</ul>', '<div class="groupsub">', '$@$for($_this_$)$@$', '<div class="groupclose" key="$oAddress.id$" sub="0">', '<a unselectable="on" nocheck="true" onmousedown="return false;" title="$oAddress.email$" class="lm_addr">', '$oAddress.name$', '</a>', '</div>', '$@$endfor$@$', '</div>']),
    _left_item: TE(['<a unselectable="on" nocheck="true" onmousedown="return false;" title="$title$" class="lm_addr">', '$beforeImage$$name$$@$if($memo$)$@$($memo$)$@$endif$@$', '</a>']),
    _right_item: T(['<a id="$rid$" title="$email$" nocheck="true" class="lm_addr lm_raddr">', '<input type="button" class="mov"/>', '<div style="overflow:hidden;height:100%;*float:right;">', '<div style="overflow:hidden;width:100%;">$name$</div>', '</div>', '</a>']),
    _research_item: TE(['<a nocheck="true" id="$pid$_s_$id$" title="$title$" style="display:$disp$" class="lm_saddr" onclick="return false">', '<div class="lm_name">$name$$@$if($memo$)$@$($memo$)$@$endif$@$</div>', '<div class="lm_email">$email$</div>', '</a>']),
    _domainMailGroupAccountIcon: T(['<img class="lm_groupAutoIcon" src="', getPath("image"), 'spacer.gif" title="', '\u90AE\u4EF6\u7EC4\u5E10\u53F7', '"/>']),
    _virtualGroupAccountIcon: T(['<img class="lm_groupUnfold" src="', getPath("image"), 'spacer.gif" title="', '\u7FA4\u7EC4\u5E10\u53F7', '"/>']),
    _addrIcon: T('<span class="$icon$"></span>'),
    _nobody: TE(['<div style="margin:4px 0 6px 17px;color:#797979">', '$@$if($locale$=="zh_CN")$@$', '\u6682\u65E0$message$', '$@$else$@$', '\u6682\u65E0', '$@$endif$@$', '</div>']),
    _search_nobody: T('<div id="$pid$noaddress" style="padding:5px;display:$disp$">\u6CA1\u6709\u7B26\u5408\u6761\u4EF6\u7684\u8054\u7CFB\u4EBA\u3002</div>'),
    _search_stranger: T('<div id="$pid$stranger" style="padding:5px;display:$disp$">\u662F\u5426\u6DFB\u52A0</div>'),
    "": null
};
QMLinkman.showDlg = function (c, a, b) {
    b = b || {};
    var g = getTop(), f = a && !a.get && a.value != "" && window.QMAddrParser ? QMAddrParser.parseAddr(a.value) : [],
        d = a && a.get ? a.get("json") : f, e = null;
    new QMDialog({
        sId: "add_address",
        sTitle: "\u4ECE\u8054\u7CFB\u4EBA\u4E2D\u6DFB\u52A0",
        sBodyHtml: T(['<div class="lm_dlg" style="background:none;">', '<div style="text-align:left; padding:0 0 6px 0;height:$height$px;">', '<div id="loading" style="text-align:center;line-height:350px">\u52A0\u8F7D\u4E2D...</div>', '<table id="content" width="93%" border="0" align="center" cellpadding="0" cellspacing="0" style="table-layout:fixed;display:none;">', '<tr>', '<td width="190" style="font-size:14px;font-weight:bold;padding:12px 0 3px 0;">\u8054\u7CFB\u4EBA<span id="totleNum"></span></td>', '<td width="20">&nbsp;</td>', '<td width="190" style="font-size:14px;font-weight:bold;padding:12px 0 3px 0;"><span id="text_name">$who$</span><span id="selectNum"></span>&nbsp;</td>', '</tr>', '<tr>', '<td><div class="bd_upload lm_cnt" id="left"></div></td>', '<td align="center"><img src="/zh_CN/htmledition/images/arrow_left.gif"/></td>', '<td><div id="right" class="bd_upload lm_cnt" style="overflow-y:auto;"></div></td>', '</tr>', '</table>', '</div>', '</div>']).replace({
            who: {
                "to": "\u6536\u4EF6\u4EBA",
                "cc": "\u6284\u9001",
                "bcc": "\u5BC6\u9001",
        ...
        sFootHtml: T(['<input type="button" class="btn wd2" id="ok" value="\u786E\u5B9A"/>', '<input type="button" class="btn wd2" id="cancel" value="\u53D6\u6D88" />']).toString(),
        nWidth: b.sDlgWidth || 480,
        bClose: true,
        onshow: function () {
            var h = this;
            waitFor(function () {
                ...
                if (i) {
                    e = new QMLinkman({
                        nType: 1,
                        bSupportGroup: b.bSupportGroup,
                        oMainGroup: b.oMainGroup || ["mailgroup", "ggroup", "domaingroup", "qqgroup"],
                        oContainer: h.S("left"),
                        oContainerRight: h.S("right"),
                        oExitAddress: b.oExitAddress || d,
                        onselect: function (j) {
