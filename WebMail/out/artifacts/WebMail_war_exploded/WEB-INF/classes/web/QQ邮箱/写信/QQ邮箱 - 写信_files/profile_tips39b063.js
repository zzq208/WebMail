(function (_aoLoadWin) {
    ...
    var _TO_RSS_URL = T("/cgi-bin/reader_article_list?sid=$sid$&t=rss_list&s=personal&classtype=onefeed&feed=3_$uin$"),
        ...
        _REJECT_MSG = T(['<div>\u62D2\u6536\u540E\u60A8\u5C06\u4E0D\u518D\u6536\u5230\u6765\u81EA\u4EE5\u4E0B\u5730\u5740\u7684\u90AE\u4EF6\uFF0C\u786E\u8BA4\u62D2\u6536\u5417\uFF1F</div>', '<div for="reject_confirm_$_idx_$" class="txtflow" style="width:320px;">', '<span class="green bold">$name$</span>', '<span class="graytext">&lt;$addr$&gt;</span>', '</div>']);
    ...
    _goClass._readyData = function (_aoTriggerDom) {
        var _oSelf = this, _oInfoDoms = [], _sUins = "", _sShowInfo = _aoTriggerDom.getAttribute("t"),
            _sRole = _aoTriggerDom.getAttribute("r"), _sMailid = _aoTriggerDom.getAttribute("mailid");
    ...
    _goClass._buildInfoCard = function () {
        var _oSelf = this, _oTop = getTop(), _sShowInfo = _oSelf._msShowInfo, _sDomId = _oSelf._msDomId,
            _sMailid = _oSelf._sMailid, _oInfos = _oSelf._moInfos, _nTargetIndex = 1, _oInfoFeeds = [], _oInfo, _sUin,
            _sEmail, _sRole;
        for (_key in _oInfos) {
            _oInfo = _oInfos[_key];
            _sUin = _oInfo._msUin;
    ...
    _goClass._fBuild = function (_aoFeeds) {
        var _oSelf = this, _oTop = getTop(), _sShowInfo = _oSelf._msShowInfo, _sEmail = _aoFeeds[0].email,
            _sUin = _aoFeeds[0].uin, _sDomId = _oSelf._msDomId, _sMailid = _oSelf._sMailid, _nTargetIndex = 1,
            _nFeedLength = _aoFeeds.length, _nTotalWidth = _nFeedLength * 320, _bIsNeedLeft = (_nTargetIndex > 1),
            _nIsNeedRigth = (_nFeedLength > 1 && _nTargetIndex != _nFeedLength),
            _nCurrentLeft = (_nTargetIndex - 1) * 320, _oData = {
                "_id": _sDomId,
                "_oFeeds": _aoFeeds,
                _nTotalWidth: _nTotalWidth,
                _bIsNeedLeft: _bIsNeedLeft,
                _nIsNeedRigth: _nIsNeedRigth,
                _nCurrentLeft: _nCurrentLeft,
                role: _oSelf._msRole
            };
            ...
            _oTop.QMAjax.send("/cgi-bin/bind?sid=" + _oTop.getSid() + "&t=linkedin_profile&fun=getlinkedintoken&useremailaddr=" + _sEmail, {
                method: "GET", onload: function (_abIsOk, _asReturn) {
                    var _oReturn = _oTop.evalValue(_asReturn),
                        ...
                        _oLinkedInProfile = _oReturn.linkedin_profile && _oTop.evalValue(decodeURI(_oReturn.linkedin_profile));
                    if (_bUseLinkedIn) {
                        ...
                        _oData._oFeeds[0] = extend({
                            "linkedInName": _oLinkedInProfile.formattedName,
                            "linkedInPublicProfileUrl": _oLinkedInProfile.publicProfileUrl,
                            "LinkedInMainAddress": _oLinkedInProfile.mainAddress,
                            "linkedInIndustry": _oLinkedInProfile.industry,
                            "linkedInTitle": _oLinkedInProfile.title,
                            "linkedInCompanyName": _oLinkedInProfile.companyName
                        }, _oData._oFeeds[0]);
                    }
                    insertHTML(_oSelf._moWin.document.body, 'afterBegin', _goStatic._TEMPLATE._INFOCARD.replace(_oData));
                    _oSelf._moInfoCard = _oInfoCard = S('infocard_' + _sDomId, _oSelf._moWin);
                    ...
                    if (_bUseLinkedIn) {
                        _oTop.bindLinkedInAccount.getLinkedInProfile(_sLinkedInAccessToken, _sEmail, function (_abIsOk, _aoProfile) {
                            if (_abIsOk) {
                                ...
                                _oTop.insertHTML(_oLinkedInInfo, "beforeBegin", _goStatic._TEMPLATE._LINKEDIN_DETAIL.replace({
                                    "linkedInName": _aoProfile.formattedName,
                                    "linkedInPublicProfileUrl": _aoProfile.publicProfileUrl,
                                    "LinkedInMainAddress": _aoProfile.mainAddress,
                                    "linkedInIndustry": _aoProfile.industry,
                                    "linkedInTitle": _aoProfile.title,
                                    "linkedInCompanyName": _aoProfile.companyName
                                }));
    ...
    _goClass._bindEvent = function (_aoCfg) {
        var _oSelf = this, _oTop = getTop(), _oInfoCard = _aoCfg.targetDom, _nTotalWidth = _aoCfg.totalWidth,
            _oFeeds = _aoCfg.datas, _oInfos = _oSelf._moInfos, _oTipAllDom = finds("div.tipAll", _oInfoCard)[0],
            _oDivLeft = finds("div.tipAllLeft", _oInfoCard)[0], _oDivRight = finds("div.tipAllRight", _oInfoCard)[0],
            _oControlEvent = {
    ...
    _goClass._buildReaderInfoCard = function (_aoInfo) {
        ...
        _oAjax.send(T("func=infocard&uin=$uin$&sid=$sid$&t=rss_mgr&s=infocard&r=$r$").replace({
            sid: getSid(),
            r: _oSelf._mbRebuildFlag === true ? Math.random() : '',
            uin: _sUin
        }));
        _oAjax.onComplete = function (_aoXml) {
            var _oJson = null;
    ...
    _goClass._moveHor = function (_aoTarget, _asDirection, _anTotalWidth, _aoDivLeft, _aoDivRight) {
        var _nCurrent = parseInt(_aoTarget.style.left.match(/(-?[\d]+)/)[0]),
            _nDest = _asDirection === "left" ? _nCurrent + 320 : _nCurrent - 320;
    ...
    _goClass._showInfoCard = function (_abShow) {
        ...
        if (_abShow && !isShow(_oInfoCard) || !_abShow && isShow(_oInfoCard)) {
            qmAnimation.play(_oInfoCard, {
                from: _abShow ? 0.5 : 1, to: _abShow ? 1 : 0.5, speed: 'fast', onaction: function (_anValue) {
    ...
    _goClass._computeCardPos = function (_asShowInfo) {
        var _oSelf = this, _oTriggerDom = _oSelf._moTriggerDom, _oInfoCard = _oSelf._moInfoCard,
            _sDomId = _oSelf._msDomId, _oWin = _oSelf._moWin, _oDocument = _oWin.document,
            _sTargetId = _oTriggerDom.getAttribute('beside'),
            _oTarget = _sTargetId ? S(_sTargetId, _oSelf._moWin) : _oTriggerDom, _oIconPos = calcPos(_oTarget),
            _oCardPos = calcPos(_oInfoCard), _oArrowUp = S('infoarrowup_' + _sDomId, _oWin),
            _oArrowDown = S('infoarrowdown_' + _sDomId, _oWin), _nClientWidth = _oDocument.body.clientWidth,
            _oQQBrowserTip = S("qqbrowser_pop");
        if (!_oInfoCard) {
            return;
        }
    ...
    _goClass._buildLogoCard = function () {
        ...
        if (!_sLogoStr) {
            ...
            var _oBuf = _sLogoStr.split("aboutlogo="), _sTipsWord = _oBuf[0], _sLink = _oBuf[1],
                _bNoLogo = _sTipsWord == "nothing" || !_sStylenum;
        }
        if (_bNoLogo) {
            ...
            for (var i = 0; i < _oArray.length; i++) {
                if (i % 3 == 0) {
                    _oLinks.push({
                        linkname: _oArray[i],
                        href: TE(_oArray[i + 1]).replace({sid: getSid(), uin: getUin()}),
                        target: _oArray[i + 2]
                    });
                }
            }
            insertHTML(_oSelf._moWin.document.body, 'afterBegin', _goStatic._TEMPLATE._LOGO.replace({
                '_id': _sDomId,
    ...
    _goClass.oprVipAddr = function (_asAction, _asAddrId, _asName, _asEmail, _afCallback) {
        var _oTop = getTop();
        _oTop.QMAjax.send("/cgi-bin/vip_addr", {
            content: _oTop.T(["sid=#sid#&action=#action#&id=#addrid#&addr=#addr#&t=vip_addr&resp_charset=UTF8"], "#").replace({
                sid: _oTop.getSid(),
                addrid: (_asAddrId == "-1" ? "" : _asAddrId),
                action: _asAction,
                addr: (_asAddrId == "" || _asAddrId == "-1") ? encodeURIComponent(["\"", _asName, "\"", " <", _asEmail, ">"].join("")) : ""
    ...
    _goStatic.oper = function (_asOper, _asUin, _asEmail, _sShowInfo, _asNickName) {
        function _toAddr(_asN, _asU, _asE) {
            return T('"$name$" <$email$>').replace({
                name: encodeNick(_asN) || _asU,
                email: _asEmail || (_asUin + "@qq.com")
            });
        }
        ...
        switch (_asOper) {
            case "mailTo":
                openComposeDlg("normal", {
                    sDefAddrs: _toAddr(_asNickName, _asUin, _asEmail),
                    bUinEncrypt: true,
                    bAddrEdit: true,
                    nWidth: 495
                });
    ...
    _goStatic.addContact = function (_aoTarget) {
        ...
        new _oTop.QMDialog({
            sId: "newAddr",
            sTitle: "\u65B0\u5EFA\u8054\u7CFB\u4EBA",
            sUrl: _oTop.T('/cgi-bin/laddr_detail?sid=$sid$&view=qq&t=contact&edit=1&user=$username$&email=$email$&dlgname=newAddr&icon=$icon$').replace({
                sid: _oTop.getSid(),
                username: encodeURI(_sUsername),
                email: encodeURIComponent(_sEmail),
                icon: encodeURIComponent("/cgi-bin/getqqicon?" + "uin=" + _sUin + "&mode=newaddr&sid=" + _oTop.getSid() + "&mailaddr=" + _sEmail)
            }),
            nHeight: 538,
            nWidth: 600,
            onload: function () {
    ...
    _goStatic.editContact = function (_aoTarget) {
        var _oTop = getTop(), _sUrl = "", _sEmail = _oTop.attr(_aoTarget, "e"), _sUsername = _oTop.attr(_aoTarget, "n"),
            _sUin = _oTop.attr(_aoTarget, "u"),
            _sUrl = T("/cgi-bin/laddr_detail?sid=$sid$&view=normal&t=contact&edit=1&AddrID=$addrid$&editexist=1&dlgname=editAddr&icon=$icon$").replace({
                sid: getSid(),
                addrid: getTop().attr(_aoTarget, "addrid"),
                icon: encodeURIComponent("/cgi-bin/getqqicon?" + "uin=" + _sUin + "&mode=newaddr&sid=" + _oTop.getSid() + "&mailaddr=" + _sEmail)
            });
        new _oTop.QMDialog({
            sId: "editAddr",
    ...
    Info.prototype._init = function (_aoTriggerDom) {
        ...
        var _oSelf = this, _sUin = _aoTriggerDom.getAttribute('u'), _sQQNum = _aoTriggerDom.getAttribute('qq'),
            _sNickName = _aoTriggerDom.getAttribute('n'), _sIsSender = _aoTriggerDom.getAttribute("sd"),
            _sEmail = _aoTriggerDom.getAttribute('e'), _sIcon = _aoTriggerDom.getAttribute('i'),
            _sRole = _aoTriggerDom.getAttribute("r"), _sQQFnd = _aoTriggerDom.getAttribute("f"),
            _sMailid = _aoTriggerDom.getAttribute("mailid"), _sHome = _aoTriggerDom.getAttribute("h"),
            _sIsOwn = _aoTriggerDom.getAttribute("io"), _sRejectHtmlId = _aoTriggerDom.getAttribute("rejecthtml") || "",
            _sFilingHtmlId = _aoTriggerDom.getAttribute("filinghtml") || "filinghtml",
            _sAddrid = _aoTriggerDom.getAttribute("addrid"), _oOperPanel = getMainWin()._oOperPanel,
            _oRejectHtmlDom = _oOperPanel ? _oOperPanel[_sRejectHtmlId] : undefined,
            _oFilingHtmlDom = _oOperPanel ? _oOperPanel[_sFilingHtmlId] : undefined,
            _sRejectHtml = _oRejectHtmlDom ? _oRejectHtmlDom : "",
            _sFilingHtml = _oFilingHtmlDom ? _oFilingHtmlDom : "", _sAddrinfoHtml = "";
        _sEmail = htmlEncode(_sEmail);
        _sAddrinfoHtml = _goStatic._TEMPLATE._EXTENDHTML.replace({
            addrid: _sAddrid,
            email: _sEmail,
            uin: _sUin,
            qqnum: _sQQNum,
            username: htmlEncode(_sNickName)
        });
    ...
    Info.prototype._imitateData = function (_aoTriggerDom) {
        var _oWin = getMainWin(), _oSelf = this, _sType = _aoTriggerDom.parentNode.getAttribute('t'),
            _oNobrDom = (_sType == "6" ? _aoTriggerDom.parentNode : _aoTriggerDom),
            _sEmail = _aoTriggerDom.getAttribute('e'), _sMailid = _oNobrDom.getAttribute('mailid'),
            _sIsGroup = _oNobrDom.getAttribute('ignore'), _sIsSender = _aoTriggerDom.getAttribute("sd"),
            _sXqqstyle = _oNobrDom.getAttribute('xqs'),
            _sAd = _oNobrDom.getAttribute('isad') == "1" || _oNobrDom.getAttribute('r') == "ad",
            _sBookcolid = _oNobrDom.getAttribute('bcd'), _sIsTuan = (_sBookcolid != 0 && _sXqqstyle == 1) ? 1 : 0,
            _sBook = 2;
        _sEmail = htmlEncode(_sEmail);
        _oWin["_oOperPanel"] = {};
        if (_sIsSender == "0" && !_isAdminEmail(_sEmail)) {
            ...
            var _sDomain = _oNobrDom.getAttribute('dm'),
                _sSysmail = (_sXqqstyle == 1 || _sXqqstyle == 4 || _sXqqstyle == 5 || _sXqqstyle == 6 || _sXqqstyle == 600) ? 1 : 0,
                _sIsbookmail = _oNobrDom.getAttribute('ibm'), _sCurrfolder = _oNobrDom.getAttribute('cfid'),
                _sBookable = _oNobrDom.getAttribute('iba'),
                _sFolderid = (_sXqqstyle == 1 || _sXqqstyle == 2 || _sXqqstyle == 6 || _sXqqstyle == 5 || _sXqqstyle == 8) ? 0 : _oNobrDom.getAttribute('fid'),
                _sCheatcode = _oNobrDom.getAttribute('ctd');
    ...
    Info.prototype._readyData = function (_aoFeed) {
        var _oFeed = _aoFeed || {}, _oTop = getTop(), _oSelf = this, _sUin = _oSelf._msUin, _sQQNum = _oSelf._msQQNum,
            _sEmail = _oSelf._msEmail, _sRole = _oSelf._msRole, _sIcon = _oSelf._msIcon, _sHome = _oSelf._msHome,
            _sIsOwn = _oSelf._msIsOwn, _sQQFnd = _oSelf._msQQFnd, _sShowInfo = _oSelf._msShowInfo,
            _sRejectionHtml = _oSelf._msRejectHtml, _sFilingHtml = _oSelf._msFilingHtml,
            _sAddrInfoHtml = _oSelf._msAddrInfoHtml, _oTriggerDom = _oSelf._moTriggerDom,
            _sNickName = _oSelf._msNickName, _nBtnNum = 0, _bAdUin = _isAdminUin(_sUin),
            _bAdminEmail = _isAdminEmail(_sEmail), _bGroupMail = _isGroupEmail(_sEmail), _sMailid = _oSelf._msMailid,
            _bMail = _sEmail && !_bAdminEmail, _bSms = _sEmail && !_bAdminEmail && !_bAdUin && !_bGroupMail,
            _bRss = _sUin && !_bAdUin && !_bAdminEmail,
            _bBanSmsByFol = _bSms && _oFeed.followedby != "1" && _sShowInfo == "2";
        ...
        var _sNickNameNE = htmlEncode(encodeNick(_oFeed.name)), _sNameHtml = htmlEncode(_wraparound(_oFeed.name)),
            _sNameAttrHtml = htmlEncode(_oFeed.name), _oDomEdit = S("a_edit_" + _sEmail, getMainWin()),
            _sNameURI = encodeURI(_oFeed.name);
        delete _oFeed.name;
        ...
        _oFeed = extend({
            ...
            'email': _sEmail,
            'dispname': _sNameHtml,
            'attrname': _sNameAttrHtml,
            'jsname': _sNickNameNE,
            'dispemail': _wraparound(_sEmail),
            'showinfo': _sShowInfo,
            'sid': getSid(),
            'images_path': getPath('image'),
            'img_path': getPath('image'),
            'btnnum': _nBtnNum,
            'arrow': _oTriggerDom.tagName == "IMG" ? 0 : 1,
            'bmail': _bMail,
            'bsms': _bSms,
            'bansms': _bBanSmsByFol,
            'brss': _bRss,
            ...
            "qqfriend": _sQQFnd,
            'home': _sHome,
            'addrinfohtml': _sAddrInfoHtml,
            'rejectionhtml': _sRejectionHtml,
            'filinghtml': _sFilingHtml,
            'name': _sNameURI,
            'nickname': _sNickName,
            'AddrId': attr(_oDomEdit, "addrid") || "",
            'addrvip': _sAddrvip ? (_sAddrvip == 'true' || _sAddrvip == '1' ? 'true' : 'false') : '',
            'addrvipid': attr(_oTriggerDom, 'addrid') || "",
            'issys': _bAdminEmail,
            'icon': "/cgi-bin/getqqicon?uin=" + _sUin + "&mode=newaddr&mailaddr=" + encodeURI(_sEmail) + "&sid=" + _oTop.getSid()
        }, _oFeed);
        return _oFeed;
    };
    Info.prototype.reject = function () {
        var _oSelf = this, _oTop = getTop(), _sMailId = _oSelf._msMailid, _sEmail = _oSelf._msEmail,
            _sName = _oSelf._msNickName, _sFolderid = _oSelf._msFolderid,
            _sMsg = _REJECT_MSG.replace({name: htmlEncode(_sName), addr: _sEmail});
        if (_oSelf._msIsSender == 0) {
            confirmBox({
                title: "\u62D2\u6536\u786E\u8BA4",
                msg: _sMsg || "\u62D2\u6536\u540E\u60A8\u5C06\u4E0D\u518D\u6536\u5230\u6765\u81EA\u6B64\u5730\u5740\u7684\u90AE\u4EF6\uFF0C\u786E\u8BA4\u62D2\u6536\u5417\uFF1F",
                confirmBtnTxt: "\u62D2\u6536",
                cancelBtnTxt: "\u53D6\u6D88",
                confirmClass: "btn_blue",
                onreturn: function (_abIsOk) {
                    if (_abIsOk) {
                        _oTop.QMAjax.send("/cgi-bin/config_blackwhitelist", {
                            content: _oTop.T(["sid=#sid#&act=blacklist&Fun=submit&confirm=1&group=#email#"], "#").replace({
                                sid: _oTop.getSid(),
                                email: encodeURIComponent(_sEmail)
    ...
    Info.prototype.exbookEmlMgr = function () {
        var _oSelf = this, _sMailId = _oSelf._msMailid, _sBook = _oSelf._msBook, _sEmail = _oSelf._msEmail,
            _sName = _oSelf._msNickName, _nIsTuan = _oSelf._msIsTuan || 0;
        loadingBox({
            model: "\u53CD\u5783\u573E", js: ["$js_path$qmantispam264eed.js"], oncheck: function () {
                ...
                var _oSpam = new QMAntiSpam.qmExbookEmlMgr({
                    sMailId: _sMailId,
                    from: {name: _sName, addr: _sEmail, qq: ""}
                });
    ...
    function _createMailFrm(_asMailId, asFolderid) {
        _createForm({oWin: getMainWin(), sFormId: "mail_frm", sAction: "/cgi-bin/mail_mgr"}, extend({
            s: "readmail_spam",
            fm: "mail_list",
            s_reject_what: "11",
            isspam: 'true',
    ...
    function _isAdminEmail(_asEmail) {
        var _oAdmin = ["10000@qq.com", "birthday@qq.com", "urlcreator@qq.com", "birthdayreminder@qq.com", "friends@qq.com"],
            _bisAdmin = false;
