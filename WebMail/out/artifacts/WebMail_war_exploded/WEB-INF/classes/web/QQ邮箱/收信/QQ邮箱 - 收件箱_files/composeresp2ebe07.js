var ComposeResp = (function (a, g) {
        ...
        h = i.inherit("_ComPOse_REsp_", Object, function (j) {
            return ({
                evt_: {
                    ...
                    switch (s) {
                        ...
                        case "gstate":
                            ...
                            if (l.sFrom == "setting") {
                                ...
                                var o = a.T('/cgi-bin/mail_list?t=$t$&sid=$sid$&folderid=8&groupid=$groupid$'),
                                    t = 'mail_list_group', m = i.S('folder_8'), n = m && m.getAttribute('href');
                        ...
                        case "reject":
                            ...
                            if (l.sFrom == "setting") {
                                ...
                                i.goUrlMainFrm(a.T("/cgi-bin/grouplist?sid=$sid$&t=compose_group&gid=$gid$&s=from_mail_list$newwin$").replace({
                                    sid: i.getSid(),
                                    gid: l.sRejectGid,
                                    newwin: i.bnewwin ? "&newwin=true" : ""
                                }), false, true);
                            }
                            else {
                        ...
                        case "card_qq":
                            ...
                            i.disableAll(false);
                            if (l.sMK == "birth") {
                                p.location.href = a.T('/cgi-bin/cardlist?t=card_qq&sid=$sid$&param=$param$&ListType=Cards&limit=18&Cate1Idx=listall&loc=cardlist,card,,1').replace({
                                    sid: i.getSid(),
                                    sParam: l.sParam
                                });
                            }
                    ...
                    var p = a.S("3GContainer"), q = a.S("notifydiv"),
                        o = parseInt(getTop().goUserInfo.get("IOSWEBAPPVER"), 10) || 0,
                        n = parseInt(getTop().goUserInfo.get("ANDROIDWEBAPPVER"), 10) || 0,
                        l = (o == "0" && n == "0") ? true : false, m = q && i.isShow(q);
                    if (l && !m) {
                    ...
                    var q = a.S("newAddr"), p = a.S("3GContainer"), v = k.oToObj, o = v && v.oItems,
                        s = k.newAddr || {}, n = k.existAddr || {}, u = k.otherexistAddr || {}, x = i.getSid(),
                        y = i.getUin(), l = v.oAddrLst = [], r = v.oNewAddrLst = [], m = v.oExistAddrLst = [],
                        t = v.oOtherExistAddrLst = [], z = "", w = [];
                    i.E(o, function (A) {
                    ...
                    if (q && (r.length > 0 || m.length > 0 || t.length > 0)) {
                        q.innerHTML = f.replace(i.extend({
                            imgpath: i.getPath('image'),
                            sid: i.getSid(),
                            b3Gshow: (p.getAttribute("isShow") == 1),
                            bADShow: i.isShow(a.S("qqmail_AD_container"))
                        }, v));
                        q.setAttribute("isShow", 1);
                        q.parentNode.style.display = "";
                    }
                }, _initWeixinData: function (k) {
                    var o = a.S("weixinAD"), m = k.oToObj, l = m && m.oItems, p = i.getSid(), q = i.getUin(), r = "",
                        n = [];
                    l && i.E(l, function (s) {
                    ...
                    i.QMAjax.send(r, {
                        method: "GET", onload: function (s, t) {
                            if (s) {
                                var A = evalValue(t), u = n.length, v = 0, x = m.oFrdWXLst = [],
                                    y = m.oNotFrdWXLst = [], z = m.oNotWXLst = [], C = "", B = "", w = new Date();
                                i.E(l, function (D) {
                                    if (D) {
                                ...
                                m.oGrpLst.pop();
                                if (o) {
                                    o.innerHTML = b.replace(i.extend({
                                        sUins: C,
                                        sAddrs: B,
                                        imgpath: i.getPath('image'),
                                        sid: i.getSid(),
                    ...
                    if (m) {
                        if (k) {
                            i.createBlankIframe(i, {
                                id: "notehandleFrame", onload: function () {
    ...
    return {
        ...
        TE: a.TE,
        attr: a.attr,
        insertHTML: a.insertHTML,
        GelTags: a.GelTags,
        addEvent: a.addEvent,
        getEventTarget: a.getEventTarget,
        isObjContainTarget: a.isObjContainTarget
    };
}());
